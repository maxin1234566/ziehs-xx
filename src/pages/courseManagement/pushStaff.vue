<template>
  <div class="push-box clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    ...
    <div class="flex">
      <div class="left border-radius-3 border-f4 pd-l-20 pd-r-20 pd-t-20 pd-b-20 m-r-20">
        ...
        <div class="staff m-b-20 flex align-items-center">
          ...
          <staffOrDept
            ...
            :checkedTreeNode="checkedStaffNode"
            @getTreeData="getTreeData"
            :treePlaceholder="'请选择组织人员'"
        ...
        <SearchTerm>
          <a-form-model layout="inline" :model="formInline" :colon="false">
            ...
            <a-form-model-item label="员工名称">
              <a-input v-model="formInline.name" placeholder="请输入员工名称"></a-input>
            </a-form-model-item>
        ...
        <div class="select-all flex justify-content-between pd-b-20">
          <div>所有厂商</div>
          <div class="font-weight-bold" v-if="companyList.length > 0" @click="clickAll">
            {{ clickAllFlag ? "全不选" : "全选" }}
          </div>
        </div>
          ...
          <a-collapse v-model="activeKey">
            ...
            >
              ...
              <CommonTable :noPaging="true">
                <a-table
                  :row-selection="{
                    selectedRowKeys: chooseStaff[item.id].selectedRowKeys,
                    onChange: (selectedRowKeys) => {
                      onSelectChange(selectedRowKeys, item.id, item.name);
                    },
                  }"
                  :columns="columns"
                  ...
                  :rowKey="
                    (record, index) => {
                      return item.name + '/' + record.id + '/' + record.name;
                    }
                  "
                  :pagination="false"
                ></a-table>
      ...
      <div
        class="right border-radius-3 border-f4 pd-l-20 pd-r-20 pd-t-20 pd-b-20"
        style="overflow: hidden"
      >
        <div class="m-b-20 font-16 font-weight-bold">
          内部人员已选（{{ checkedUserTitle.length }}）人
        </div>
        <CommonTable :noPaging="true">
          <a-table
            :columns="columnsDetail1"
            :locale="{ emptyText: emptyText }"
            :data-source="checkedUserTitle"
            :pagination="false"
          ></a-table>
        </CommonTable>
        <div class="m-b-20 font-16 font-weight-bold">
          厂商人员已选（{{ chooseStaffDetail.length }}）人
        </div>
        <CommonTable :noPaging="true">
          <a-table
            :columns="columnsDetail"
            :locale="{ emptyText: emptyText }"
            :data-source="chooseStaffDetail"
            :pagination="false"
          ></a-table>
        </CommonTable>
      </div>
...
export default {
  ...
  created() {
    ...
    this.deptTreeId = this.getMappingValue(
      this.getCommonAddOrgnizeListAll,
      "orgId",
      this.dataMsg.corporationId
    ).deptId;
  ...
  methods: {
    ...
    getTreeData(obj) {
      ...
      let brr = [];
      console.log("obj.treeNameAndCodeList", obj.treeNameAndCodeList);
      ...
      for (let key in this.chooseStaff) {
        for (let j = 0; j < this.chooseStaff[key].selectedRowKeys.length; j++) {
          for (let k = 0; k < this.pushList.length; k++) {
            if (
              this.pushList[k].id ==
              this.chooseStaff[key].selectedRowKeys[j].split("/")[1]
            ) {
              arr.push({
                ...this.pushList[k],
                companyId: key,
          ...
          for (let i = 0; i < this.companyList.length; i++) {
            if (this.companyList[i].id == key) {
              for (let j = 0; j < this.companyList[i].staffList.length; j++) {
                this.chooseStaff[key].selectedRowKeys.push(
                  this.companyList[i].staffList[j].id
                );
              }
            }
          }
