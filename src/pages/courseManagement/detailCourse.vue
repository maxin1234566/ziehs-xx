...
<script>
import { GetSubjectlist, CourseDetail, PushInfo, PushInfoExport, PushCodeInfo, PushCodeInfoExport, deptDict, courseManagementDetailCourseDowloadApi } from "@/services/api.js";
  ...
  methods: {
    ...
    iExportDetail() {
      return PushCodeInfoExport({
        ...
        const href = URL.createObjectURL(blob); //创建下载链接
        downloadElement.href = href;
        downloadElement.download = '学习情况' + '.xlsx';
        document.body.appendChild(downloadElement);
        downloadElement.click();
        document.body.removeChild(downloadElement);// 下载完成移除元素
        window.URL.revokeObjectURL(href) // 释放掉blob对象
