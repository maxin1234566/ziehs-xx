<template>
  <HasFixedBottomWrapper>
    <div class="tip">
      <i>!</i
      >请您遵守国家相关规定，切勿上传低俗色情、暴力恐怖、谣言诈骗、侵权盗版等相关内容，工业互联网平台保有依据国家规定及平台规则进行处理的权利。
    </div>
    ...
    <div class="m-t-20">
      <FormPage
        ref="basicForm"
        :formPageModal="basicInfoForm"
        :formPageRules="basicInfoRules"
      >
        <template slot="left">
          <NewCommonSearchItem
            :CommonFormInline="basicInfoForm"
    ...
    <div class="m-t-20">
      <FormPage
        ref="courseForm"
        :formPageModal="courseInfoForm"
        :formPageRules="courseInfoRules"
      >
        <template slot="left">
        ...
        <template slot="right">
          <a-form-model-item label="选择讲师">
            <a-select
              mode="multiple"
              v-model="courseInfoForm.lecturerIdList"
              placeholder="请选择讲师"
            >
              <a-select-option
      ...
      <CommonTable :noPaging="true">
        ...
        >
          <div slot="action" slot-scope="record">
            <span class="color-0067cc cursor-pointer m-r-15" @click="actionEdit(record)"
              >编辑</span
            >
            <span class="color-0067cc cursor-pointer" @click="actionDelete(record)"
              >删除</span
            >
          </div>
          <div slot="coursewareList" slot-scope="record">
            <span class="m-r-8" v-for="item in record.coursewareList" :key="item">{{
              findText(coursewareListAll, "coursewareId", item).name
            }}</span>
          </div>
        </a-table>
      </CommonTable>
    ...
    <div class="m-t-20">
      <FormPage ref="testForm" :formPageModal="testForm">
        <template slot="left">
          ...
          <div
            class="flex justify-content-between test-paper pd-l-20 pd-r-20 pd-t-20 pd-b-20 bg-f5f7fa border-radius-3"
            v-show="testForm.paperId"
          >
      ...
      <template slot="form">
        ...
        >
          ...
          <a-form-model-item class="flex" label="模块描述">
            <a-textarea
              v-model="addModleForm.description"
              :maxLength="300"
              placeholder="请输入模块描述"
            />
          </a-form-model-item>
          <a-form-model-item class="flex" label="模块资源" prop="coursewareList">
            <a-select
              v-model="addModleForm.coursewareList"
              :filterOption="filterOption"
              mode="multiple"
              style="width: 100%"
              placeholder="请选择课件资源，可多选"
            >
              <a-select-option
                v-for="item in coursewareListAll"
      ...
      <div class="flex">
        <div class="test-left border-f4 pd-l-20 pd-r-20 pd-t-20 pd-b-20 bg-fc m-r-20">
          ...
          <div class="main pd-l-10 pd-t-20 pd-r-10">
            <div
              class="m-b-16"
              :class="{ 'color-0067cc': !testForm.subjectId }"
              @click="chooseTestType()"
            >
              所有试卷
            </div>
            <div class="type-list">
...
export default {
  ...
  data() {
    return {
      ...
      basicInfoRules: {
        name: [{ required: true, message: "课件名称不能为空", trigger: "blur" }],
        cover: [{ required: true, message: "课程封面不能为空", trigger: "change" }],
        patch: [{ required: true, message: "课程贴片不能为空", trigger: "change" }],
      },
      // 课程信息
      courseInfoForm: {
        moduleList: [],
      ...
      courseInfoRules: {
        ...
        moduleList: [{ required: true, message: "课程模块不能为空", trigger: "change" }],
      },
      moduleVisible: false,
      modalTitle: "",
      addModleForm: {
  ...
  async created() {
    ...
    if (this.$route.params.dataMsg) {
      ...
      let result1 = await CourseDetail({
        courseId: this.dataMsg.courseId,
        subjectId: this.dataMsg.subjectId,
      });
      result1.data.lecturerList = result1.data.lecturerList
        ? result1.data.lecturerList
        : [];
      ...
      if (result1.data.paper) {
        this.testForm.paperId = result1.data.paper[0]
          ? result1.data.paper[0].paperId
          : "";
  ...
  methods: {
    ...
    handleSuccess1() {
      this.basicInfoForm.patch =
        this.$refs.coursePatch.fileList.length > 0
          ? this.$refs.coursePatch.fileList[0].id
          : "";
      this.basicInfoForm = { ...this.basicInfoForm };
      formValidator.formItemValidate(this.$refs.basicForm, "patch", "addForm");
    },
    handleSuccess2() {
      this.basicInfoForm.cover =
        this.$refs.courseCover.fileList.length > 0
          ? this.$refs.courseCover.fileList[0].id
          : "";
      this.basicInfoForm = { ...this.basicInfoForm };
    ...
    confirmSubmit() {
      ...
      if (
        !(
          formValidator.formAll(this.$refs.basicForm, "addForm") &&
          formValidator.formAll(this.$refs.courseForm, "addForm") &&
          formValidator.formAll(this.$refs.sortForm, "addForm")
        )
      ) {
        return;
      }
      let param = {
        ...
        // ...this.groundingForm,
        format: 1,
        coverUrl: this.$refs.courseCover.fileList[0]
          ? this.$refs.courseCover.fileList[0].url
          : "",
      };
