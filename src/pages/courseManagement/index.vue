  ...
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
      ...
      <a-form-model layout="inline" :model="formInline" :colon="false">
        ...
        <a-form-model-item label="课程名/创建人">
          <a-input
            ...
            placeholder="请输入课程名称和创建人"
            allowClear
          ></a-input>
        </a-form-model-item>
        <a-form-model-item label="科目">
          <a-select v-model="formInline.subjectId" placeholder="请选择科目" allowClear>
            <a-select-option
    ...
    >
      ...
      >
        <div slot="action" slot-scope="record">
          <!-- :class="{
              'color-999': record.status != 1,
              'cursor-disabled': record.status != 1,
            }" -->
          <span
            class="color-0067cc cursor-pointer m-r-15"
            @click="actionPushStaff(record)"
            >推送</span
          >
          <!-- <span class="color-0067cc cursor-pointer m-r-15" @click="actionRecall(record)">撤回</span> -->
          <span class="color-0067cc cursor-pointer m-r-15" @click="actionLook(record)"
            >查看</span
          >
          <span
            class="color-0067cc cursor-pointer m-r-15"
            :class="{
              'color-999': record.pushStatus == 2,
              'cursor-disabled': record.pushStatus == 2,
            }"
          ...
          <!-- <span
            ...
            >{{ record.status == 0 ? "上架" : "下架" }}</span> -->
        </div>
        <div slot="subjectId" slot-scope="record">
          <a-popover autoAdjustOverflow>
            <div slot="content">
              <p>
                {{ findText(subjectListAll, "subjectId", record.subjectId).name }}
              </p>
            </div>
            <span>{{
              findText(subjectListAll, "subjectId", record.subjectId).name
    ...
    >
      ...
      <template slot="btn1">
        ...
        <a-button type="primary" class="m-l-15" :loading="loadingTwo" @click="pushConfirm"
          >确认推送</a-button
        >
      </template>
    </CommonModal>
  ...
  methods: {
      ...
      return GetCourseList(params)
        .then((res) => {
          ...
          for (let i = 0; i < this.tableDataList.length; i++) {
            this.tableDataList[i].rowOrder =
              (this.page.pageNo - 1) * this.page.pageSize + i + 1;
          }
    ...
    pushConfirm() {
      ...
      for (let i = 0; i < this.checkedUserTitle.length; i++) {
        this.checkedUserTitle[i].deptName = this.findDeptName(
          this.userTreeData,
          this.checkedUserTitle[i].deptId
        );
      }
      // 内部
      ...
      console.log("userList1传过来的值666", userList1);
      const userList = userList1.filter((item) => item.isLeaf);
      const deptList = userList1.filter((item) => !item.isLeaf);
      ...
      if (
        companyList1.length &&
        companyList1[0].userList &&
        companyList1[0].userList.length
      ) {
        // return
        let apiData = {
          companyId: companyList1[0].companyId,
          userIdList: companyList1[0].userList
      ...
      // 推送接口
      const PushCourseFn = () => {
        // console.log(companyList1, companyList2, '999999999999999999999999999999999');
        let nbList = JSON.parse(JSON.stringify(companyList1));
        console.log(nbList, "nbListnbListnbList");
        if (nbList.length > 0) {
          const deptList = nbList[0].deptList.filter(
            (item) => !item.workNum && !item.userCode
          );
          const personList = nbList[0].deptList.filter(
            (item) => item.workNum && item.userCode
          );
          // console.log(nbList,'nbListnbListnbList')
          // console.log(deptList,'deptListdeptListdeptList')
          // console.log(personList,'personListpersonListpersonListpersonList')
          nbList[0].deptList = deptList;
          // console.log(nbList,'nbListnbListnbList')
          const list = [...nbList[0].userList, ...personList];
          // console.log(list,'listlistlist')
          nbList[0].userList = list;
          // console.log(nbList,'nbListnbListnbList')
          // console.log(deptList,'deptListdeptListdeptList')
          // console.log(personList,'personListpersonListpersonListpersonList')
        }
        PushCourse({
          ...
          companyList: [...nbList, ...companyList2],
        })
          .then((res) => {
