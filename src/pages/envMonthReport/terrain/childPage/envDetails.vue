<template>
  ...
  <HasSpinAndBtnBox :spinning="spinning">
      ...
      <div class="content-box-l" :class="{ 'content-box-l2': this.status != '01' }">
        <SearchTerm>
          <a-form-model layout="inline" :model="searchFormData" :colon="false">
            ...
            >
              <a-select
                v-model="searchFormData.corporationId"
                placeholder="请选择所属组织"
            ...
            <a-form-model-item label="填报月份">
              <el-date-picker
                style="width: 200px"
                v-model="searchFormData.monthlyDate"
        ...
        >
          ...
          <vxe-column
            ...
            field="dataItem"
            width="200"
            :title="searchFormData.monthlyDate"
            :edit-render="{ placeholder: '请输入数值' }"
          >
            <template #edit="scope">
              <vxe-input
                v-model.number="scope.row.dataItem"
                :disabled="
                  !(scope.row.nicheItemsCode == 1123 || scope.row.nicheItemsCode < 1087)
                "
                type="number"
                transfer
                @change="dataItemChange(scope)"
              ></vxe-input>
            </template>
          </vxe-column>
          <vxe-column
    ...
    <FixedBottom slot="fixedBottom">
      <template v-if="!manage">
        ...
        <a-button
          class="m-r-15"
          type="primary"
          v-if="this.status == '02' && userId == createUserId"
          @click="handleWithdraw"
          >撤回</a-button
        >
        <a-button class="m-r-15" type="primary" v-if="type != 2" @click="validEvent(1)"
          >保存</a-button
        >
        <a-button class="m-r-15" type="primary" v-if="type != 2" @click="validEvent(2)"
          >提交</a-button
        >
      </template>
      <template v-else>
    ...
    <CommonModal
      :title="typeOfApproval == 1 ? '驳回原因' : '通过原因'"
      :visible="submitView"
    ...
    >
      <template slot="form">
        ...
        >
          <a-form-model-item
            class="flex"
            :label="typeOfApproval == 1 ? '驳回原因' : '通过原因'"
          >
            <a-textarea
              v-model="cause"
              placeholder="请输入"
...
import {
  ...
  update,
  envMonthApproved,
  envMonthApprovalRejection,
  envMonthApprovalWithdraw,
  getLog,
...
export default {
  ...
  created() {
    ...
    this.showBtn = this.$route.query.showBtn == 2 ? 2 : 1;
  ...
  computed: {
    ...
    v1088NaturalGasBase() {
      ...
      if (
        corporationId == "1586691773627400194" ||
  ...
  methods: {
    ...
    validEvent: debounce(
      function (n) {
        ...
        this.$refs.xTable.validate().then((res) => {
          // console.log(res);
          console.log("this.type", this.type); // 新建 1 查看 2 修改 3
          if (!res) {
            if (this.type == 1) {
              checkOut({
    ...
    submitData(n) {
      let apiData = {
        ...
        centerId: this.searchFormData.centerId, //中心id
        corporationId: this.searchFormData.corporationId, //组织机构id
        environmentMonthlyItemList: this.tableData
          .filter((i) => i.nicheItemsCode > 1000)
          .map((i, index) => {
            i.index = index;
            return i;
          }), //环境月报数据集合
      ...
      saveEnvMonth(apiData)
        .then((res) => {
          this.$antMessage.success("填报成功！"); // 新增-提交 需要monthlyId nextHandler push
          if (apiData.status == "02") {
            const url =
              ...
              "&routeUrl=" +
              `/environmentManage/envMonthReport/envDetails&type=2&manage=true&monthlyId=${res.data.monthlyId}`;
            PushTask({
              title: this.searchFormData.monthlyDate + "环境月报审批",
              approval: "envMonthReport",
              // userId: '10313136', //res.data.nextHandler
    ...
    editData(n) {
      ...
      update(apiData)
        .then((res) => {
          this.$antMessage.success("成功！"); // 编辑-提交 需要monthlyId nextHandler push
          if (apiData.status == "02") {
            const url =
              process.env.VUE_APP_LOGIN_URL +
              "client_id=" +
    ...
    handleConfirm: debounce(
      function () {
        if (this.typeOfApproval == 2) {
          envMonthApproved({
            monthlyId: this.monthlyId,
            handlingOpinions: this.cause || "",
          }).then((res) => {
            this.$antMessage.success("审批成功！");
            if (res.data.nextNodeType != "03") {
              const url =
                process.env.VUE_APP_LOGIN_URL +
                "client_id=" +
                process.env.VUE_APP_CLIENTID +
    ...
    getDataItem(nicheItemsCode) {
      let dataItemObj = this.tableData.find(
        (item) => item.nicheItemsCode == nicheItemsCode
      );
      let dataItem = dataItemObj ? dataItemObj.dataItem : 0;
      return dataItem;
    },
    getDataItemIndex(nicheItemsCode) {
      let dataItemIndex = this.tableData.findIndex(
        (item) => item.nicheItemsCode == nicheItemsCode
      );
