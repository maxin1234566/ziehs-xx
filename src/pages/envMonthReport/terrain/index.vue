<template>
  ...
  <div class="searchtable-wrapper clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    ...
    <DashBtn>
      ...
      <div>
        <a-button type="primary" @click="handleToImport" class="m-l-20"
          >批量导入</a-button
        >
        <a-button type="primary" @click="handleTodownload" class="m-l-20"
          >下载模板</a-button
        >
        <a-button type="primary" @click="handleToexport" class="m-l-20"
          >批量导出</a-button
        >
      </div>
    </DashBtn>
    ...
    >
      ...
      >
        <div slot="userName" slot-scope="record">
          <span>{{ record.userName + "/" + record.userCode }}</span>
        </div>
        <div slot="action" slot-scope="record">
          <span class="color-0067cc cursor-pointer" @click="handleCheckInfo(record)"
            >查看</span
          >
          <span
            class="color-0067cc cursor-pointer"
            v-if="
              (record.status == '01' || record.status == '04') &&
          ...
          <span
            ...
            v-if="record.status == '02' && record.handleUser == userId"
            >处理</span
          >
        </div>
      </a-table>
      ...
      <template slot="form">
        ...
        >
          <NewCommonSearchItem
            ...
            labelAlign="left"
            :notTablePage="true"
            :CommonFormInline="exportForm"
          ></NewCommonSearchItem>
          <a-form-model-item class="flex" label="导出月份" prop="month">
            <el-date-picker
              v-model="exportForm.monthArr"
              type="monthrange"
              range-separator="-"
              start-placeholder="开始月份"
              end-placeholder="结束月份"
          ...
          <a-form-model-item class="flex" label=" ">
            <a-button
              ...
              :disabled="!exportForm.corporationId"
              >导出</a-button
            >
          </a-form-model-item>
        </a-form-model>
...
export default {
  ...
  data() {
    return {
      ...
      actions: `${process.env.VUE_APP_API_BASE_URL}/ziehs-environment/api/ehs/month/environmentMonth/importFile`,
      addFormRules: {
        monthlyDate: [{ required: true, message: "月份不能为空", trigger: "change" }],
        centerId: [{ required: true, message: "中心不能为空", trigger: "change" }],
  ...
  methods: {
    ...
    initData() {
      ...
      if (this.searchFormData.timeArr) {
        this.searchFormData.monthlyDateStart = this.searchFormData.timeArr[0]
          ? dayJs(this.searchFormData.timeArr[0]).format("YYYY-MM")
          : "";
        this.searchFormData.monthlyDateEnd = this.searchFormData.timeArr[1]
          ? dayJs(this.searchFormData.timeArr[1]).format("YYYY-MM")
          : "";
      }
    ...
    //下载模板
    handleTodownload(e) {
      DownLoadFiles({ id: '1822202349109526528' }).then((res) => {
        const blob = new Blob([res.data]);
        const objectURL = URL.createObjectURL(blob);
        let btn = document.createElement("a");
        btn.download = "环境能源月报导入模板.xlsx";
        btn.href = objectURL;
        btn.click();
        URL.revokeObjectURL(objectURL);
        btn = null;
      });
      // window.open(`${process.env.VUE_APP_API_FILEUPLOAD}/file/template/enviromentImport.xlsx`);
    },
    //批量导出
    handleToexport() {
      console.log("批量导出");
    ...
    handleSubmit: debounce(
      function (e) {
        envMonthSubmit({ monthlyId: e.monthlyId }).then((res) => {
          console.log(res, "直接点击提交审批后返回的数据");
          const url =
            ...
            "&routeUrl=" +
            `/environmentManage/envMonthReport/envDetails&type=2&manage=true&monthlyId=${e.monthlyId}`;
          PushTask({
            title: e.monthlyDate + "环境月报审批",
            approval: "envMonthReport",
            userId: res.data.nextHandler,
            // userId: '10313136',  // 这个是测试用的 在移动门户测试端上
            url: url,
            docId: res.data.monthlyId, //主键id
          });
          this.$antMessage.success("提交成功！"); // 提交审批  需要monthlyId nextHandler push
          this.iRest();
        });
      },
      2000,
    ...
    handleExportData() {
      if (this.exportForm.monthArr && this.exportForm.monthArr.length) {
        this.exportForm.monthlyDateStart = dayJs(this.exportForm.monthArr[0]).format(
          "YYYY-MM"
        );
        this.exportForm.monthlyDateEnd = dayJs(this.exportForm.monthArr[1]).format(
          "YYYY-MM"
        );
      }
