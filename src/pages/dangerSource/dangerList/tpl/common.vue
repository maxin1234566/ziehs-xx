<template>
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        <a-form-model-item label="编号">
          <a-input v-model="formInline.code" placeholder="请输入" allowClear></a-input>
        </a-form-model-item>
        <a-form-model-item label="所属分类">
          <a-select
            ...
            allowClear
          >
            <a-select-option v-for="(val, key) in classDictObj" :key="key" :value="key">{{
              val
            }}</a-select-option>
          </a-select>
        </a-form-model-item>
        <a-form-model-item label="所属组织">
          <a-select v-model="formInline.corporationId" placeholder="请选择" allowClear>
            <a-select-option
        ...
        <a-form-model-item label="危险源种类">
          <a-select v-model="formInline.riskClass" placeholder="请选择" allowClear>
            <a-select-option
        ...
        <a-form-model-item label="可能导致事件">
          <a-select v-model="formInline.possibleEvents" placeholder="请选择" allowClear>
            <a-select-option
        ...
        <a-form-model-item label="关键词">
          <a-input v-model="formInline.keyWord" placeholder="请输入" allowClear></a-input>
        </a-form-model-item>
    ...
    <CommonModal title="导入-共性危险源" :visible="addVisible" :cancelFn="addCancle">
      <template slot="form">
        <a-form-model
          ref="addForm"
          :model="addForm"
        ...
        >
          ...
          <a-form-model-item class="flex" label="所属分类" prop="classificationCode">
            <a-select
...
import { queryDeptTree, DownLoadFiles } from "@/services/api.js";
...
export default {
  ...
  data() {
    return {
      ...
      //导入弹窗开关
      addVisible: false,
      actions: `${process.env.VUE_APP_API_BASE_URL}/ziehs-safeManage/api/ehs/risk/detail/public/importUser`,
      formInline: {
  ...
  methods: {
    ...
    //下载模板
    downTemplate() {
      DownLoadFiles({ id: "1822110851470868480" }).then((res) => {
        const blob = new Blob([res.data]);
        const objectURL = URL.createObjectURL(blob);
        let btn = document.createElement("a");
        btn.download = "共性危险源导入模板.xlsx";
        btn.href = objectURL;
        btn.click();
      ...
      // window.open(
      //   `${process.env.VUE_APP_API_FILEUPLOAD}/file/template/riskDetaild.xlsx`
      // );
    },
    ...
    earlyWarningTest() {
      //弹窗需控制  页面不需要
      this.$router.push({
        path: "/safeManage/dualControlManage/riskManage/classification",
      });
    },
