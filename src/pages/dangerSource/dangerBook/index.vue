  ...
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        ...
        <a-form-model-item label="状态">
          <a-select
            v-model="formInline.infoStatus"
            placeholder="请选择作业状态"
            allowClear
          >
            <a-select-option
              v-for="item in wxyStatus"
              :key="item.key"
              :value="item.key"
              >{{ item.value }}</a-select-option
            >
        ...
        <a-form-model-item label="危险源种类">
          <a-select
            v-model="formInline.riskClass"
            placeholder="请选择危险源种类"
            allowClear
          >
            <a-select-option
              v-for="item in wxyHazardkindDictList"
              :key="item.dictValue"
        ...
        <a-form-model-item label="可能导致事件">
          <a-select
            v-model="formInline.possibleEvents"
            placeholder="请选择可能导致事件"
            allowClear
          >
            <a-select-option
              v-for="item in wxyLeadtheeventDictList"
              :key="item.dictValue"
              :value="item.dictValue"
              >{{ item.dictLabel }}</a-select-option
            >
    ...
    <DashBtn>
      ...
      <div>
        ...
        <a-button type="dashed" :disabled="reviewDisabled" @click="expertReview"
          >专家评审</a-button
        >
      </div>
      <div>
        <a-button type="primary" class="btn m-l-20" @click="inClick">导入</a-button>
        <a-button type="primary" class="btn m-l-20" @click="downTpl" :loading="exportLoad"
          >导出</a-button
        >
      </div>
    </DashBtn>
    ...
    >
      ...
      >
        ...
        <div slot="action" slot-scope="record">
          <span
            v-if="record.infoStatus == 'status1' || record.infoStatus == 'status4'"
            class="color-0067cc cursor-pointer m-r-15"
            @click="actionEdit(record.id)"
            >编辑</span
          >
          <span
            v-if="record.infoStatus == 'status2' || record.infoStatus == 'status3'"
            class="color-0067cc cursor-pointer m-r-15"
            @click="actionPreview(record.id)"
            >查看</span
          >
          <span
            v-if="
              record.infoStatus == 'status2' ||
              record.infoStatus == 'status3' ||
              record.infoStatus == 'status4'
            "
            class="color-0067cc cursor-pointer m-r-15"
        ...
        >
          ...
          <a-form-model-item class="flex" label="通知模板" prop="template">
            <a-textarea
              allowClear
              placeholder="请输入通知模板"
              v-model="reviewForm.template"
              :auto-size="{ minRows: 2, maxRows: 6 }"
            />
          </a-form-model-item>
          <a-form-model-item class="flex experts-item" label="专家列表" prop="experts">
            <div style="width: 300px; height: 30px; color: #999">
              以下所属组织均为必填项
            </div>
          </a-form-model-item>
          ...
          <staffOrDept
            ...
            :checkedTreeNode="
              reviewForm[item.corporationId]
                ? reviewForm[item.corporationId].split(',')
                : []
            "
...
import {
  ...
  accountDelete,
  accountExport,
  selectClassByCorporation,
  reviewSave,
  reviewExpert,
} from "@/services/dangerSource/dangerBook/index.js";
import { PushTask, DownLoadFiles } from "@/services/api";
export default {
  mixins: [
  ...
  data() {
    return {
      wxyHazardkindDictList: [],
      wxyLeadtheeventDictList: [],
      tableSpinning: false,
      centerAreaList: [],
      tableRowKey: "id",
      classificationName: "",
      ...
      detailVisible: false,
      reviewTitle: "新增",
      actions: `${process.env.VUE_APP_API_BASE_URL}/ziehs-safeManage/api/ehs/risk/detail/account/importFile`,
      formInline: {},
      dangerBookForm: {},
      reviewForm: {
        title: "危险源评审",
      ...
      addFormRules: {
        ...
        deptId: [{ required: true, message: "组织编码不能为空", trigger: "change" }],
        orgAbbrName: [{ required: true, message: "机构简称不能为空", trigger: "blur" }],
        orgName: [{ required: true, message: "机构全称不能为空", trigger: "blur" }],
      },
    };
  },
  created() {
    this.setRouterCode("dangerBook");
    this.wxyHazardkindDictList = this.getChemicalDictList("wxy_hazardkind");
    this.wxyLeadtheeventDictList = this.getChemicalDictList("wxy_leadtheevent");
  ...
  computed: {
    ...
    reviewFormRules() {
      const reviewFormRules = {
        subject: [{ required: true, message: "不能为空", trigger: ["blur", "change"] }],
        template: [{ required: true, message: "不能为空", trigger: ["blur", "change"] }],
        experts: [
          {
            required: true,
            validator: this.expertsValidator,
            trigger: ["blur", "change"],
          },
        ],
        operations: [
  ...
  methods: {
    ...
    // 下载模板
    downloadTemplate() {
      DownLoadFiles({ id: '1822116483955572736' }).then((res) => {
        const blob = new Blob([res.data]);
        const objectURL = URL.createObjectURL(blob);
        let btn = document.createElement("a");
        btn.download = "危险源台账导入模板.xlsx";
        btn.href = objectURL;
        btn.click();
        URL.revokeObjectURL(objectURL);
        btn = null;
      });
      // window.open(`${process.env.VUE_APP_API_FILEUPLOAD}/file/template/riskAccount.xlsx`);
    },
    // 批量导入成功
    handleSuccess() {
      this.iRest();
    ...
    expertReview() {
      ...
      if (visible) {
        ...
        selectClassByCorporation({ corporationId: id })
          .then((res) => {
            if (!res.data) {
              this.$antMessage.error(
                "未匹配到所属组织对应的所属分类，请维护分类配置后重试！"
              );
            ...
            this.expertsList = [];
            for (let key in data) {
              const dept = this.corporationList.find((item) => item.orgId == key);
              const corp = {
                corporationId: key,
                corporationName: dept.orgName,
    ...
    async pushTask(id, code, userIds) {
      const url =
        ...
        "&routeUrl=" +
        `/safeManage/dualControlManage/riskManage/processReview&dangerId=${id}&dangerCode=${code}`;
    ...
    actionEdit(id) {
      this.$router.push({
        path: "/safeManage/dualControlManage/riskManage/addEditDangerBook",
        query: { id },
      });
    },
    // 查看
    actionPreview(id) {
      this.$router.push({
        path: "/safeManage/dualControlManage/riskManage/previewDangerBook",
        query: { id },
      });
    },
    // 评审详情
