    ...
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        ...
        <a-form-model-item label="状态">
          ...
          >
            <a-select-option
              v-for="item in dictionary('riskPointApproveStatus')"
              :key="item.key"
              :value="item.key"
              >{{ item.value }}</a-select-option
            >
      ...
      >
        <div slot="action" slot-scope="text, record">
          ...
          <span
            ...
            v-if="[2, 3].includes(record.auditStatus - 0)"
            @click="goHandleOrShowPge('show', record)"
            >查看</span
          >
          <span
            class="color-0067cc cursor-pointer"
            v-if="[1, 4].includes(record.auditStatus - 0)"
            @click="goAddChangePage('change', record)"
            >编辑</span
          >
          <span
            class="color-ff4d4f cursor-pointer"
            v-if="[1, 3, 4].includes(record.auditStatus - 0)"
            @click="delDataList(record)"
            >删除</span
          >
          <span
            class="color-0067cc cursor-pointer"
            v-if="isResolveVisible(record)"
            @click="goHandleOrShowPge('handle', record)"
            >处理</span
          >
        </div>
      </a-table>
...
import {
  getRiskApprovalListApi,
  rmRiskApprovalItemApi,
  ...
  methods: {
    ...
    getTableList() {
      ...
      const params = {
        ...
        auditTime: undefined,
        createTimeStart:
          auditTime && auditTime[0]
            ? dayJs(auditTime[0]).format("YYYY-MM-DD")
            : undefined,
        createTimeEnd:
          auditTime && auditTime[1]
            ? dayJs(auditTime[1]).format("YYYY-MM-DD")
            : undefined,
    ...
    goHandleOrShowPge(type, record) {
      let path =
        type == "handle"
          ? "/safeManage/dualControlManage/riskManage/riskApprovalHandle"
          : "/safeManage/dualControlManage/riskManage/riskApprovalShow";
    ...
    goAddChangePage(type, record) {
      let path =
        type == "add"
          ? "/safeManage/dualControlManage/riskManage/riskApprovalAdd"
          : "/safeManage/dualControlManage/riskManage/riskApprovalEdit";
      let query =
        type == "add" ? {} : { riskPointApprovalId: record.riskPointApprovalId };
      this.$router.push({
        path,
        query,
      });
    ...
    delDataList(record) {
      this.$antConfirm({
        title: "确定删除吗?",
        onOk: () => {
          return rmRiskApprovalItemApi({
            riskPointApprovalId: record.riskPointApprovalId,
          })
            .then((res) => {
