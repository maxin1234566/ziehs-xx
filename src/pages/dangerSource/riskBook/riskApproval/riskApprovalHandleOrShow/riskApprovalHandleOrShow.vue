...
export default {
  ...
  created() {
    ...
    this.riskPointApprovalId =
      this.$route.query.riskPointApprovalId || getQueryVariable("riskPointApprovalId");
  },
  mounted() {
    // 页面初始化
    this.initPage();
        ...
        return getRiskUserNodeInfo(para)
          .then((res) => {
            ...
            this.plainOptions = (data || []).map((item) => {
              const { userId: value, userName: label } = item;
              return { value, label };
            });
          })
          .catch((err) => {});
      ...
      return new Promise((resolve, rej) => {
        getRiskApprovalDetailApi(apiData)
          .then((res) => {
            ...
            this.iFrom = iFrom;
            if (iFrom.processId) {
              this.getflowLog(iFrom.processId, iFrom.handler).then((res) => {
                resolve(this.iFrom);
              });
            } else {
              resolve(this.iFrom);
