<template>
  <div class="searchtable-wrapper clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    ...
    <CommonDrawer
      title="选择风险点"
      :visible="selRiskPointShow"
      :cancelFn="closeSelRiskPoint"
      :width="'90vw'"
    >
      <template>
        <SelRiskPoint
          ...
          :selectedRowOld="iFrom.riskPointList || []"
        />
      </template>
      <template slot="btn">
...
import {
  addRiskApprovalApi,
  getRiskApprovalDetailApi,
  changeRiskApprovalApi,
    ...
    submitChemicalIdentSafetyTips(handler) {
      ...
      return apiName(apiData)
        .then((res) => {
          ...
          // 代办推送
          let riskPointApprovalId = this.isAddPage
            ? res.data?.id
            : this.riskPointApprovalId;
    ...
    async pushTask(securityUser, riskPointApprovalId) {
      if (riskPointApprovalId) {
        const url =
          ...
          "&routeUrl=" +
          `/safeManage/dualControlManage/riskManage/riskApprovalHandle&riskPointApprovalId=${riskPointApprovalId}`;
