<template>
  <div class="searchtable-wrapper clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
          ...
          >
            <template title="基本信息">
              ...
              <a-row>
                <a-col :span="13">
                  <a-form-model-item
                    ref="riskPointLocation"
                    label="所在位置"
                    prop="riskPointLocation"
                  >
                    <a-input
                      v-model="iFrom.riskPointLocation"
                      :maxLength="30"
                      placeholder="请输入所在位置"
                    ></a-input>
                  </a-form-model-item>
                ...
                <a-col :span="13">
                  <a-form-model-item ref="riskLevel" label="风险分级" prop="riskLevel">
                    <a-select
                      ...
                      placeholder="请先关联危险源"
                      :disabled="
                        !iFrom.dangerSourceList || !iFrom.dangerSourceList.length
                      "
                    ...
                    >
                      <a-select-option
                        v-for="item in dictionary('riskLevel')"
                        :key="item.key"
                        :value="item.key"
            ...
            <template title="关联危险源">
              <div>
                <div class="ttile border-b-e7">
                  <div
                    class="ttile-text"
                    :class="[
                      iRules.dangerSourceList[0].required ? 'ttile-text-required' : '',
                    ]"
                  >
                    关联危险源
                  </div>
                  <DashBtn class="ttile-bbtn">
                    <div>
                      <a-button type="dashed" @click="openHazardSourceDrawer()"
                        >选择危险源</a-button
                      >
                    </div>
                  </DashBtn>
              ...
              <a-form-model-item
                ref="dangerSourceList"
                label=" "
                prop="dangerSourceList"
                :label-col="{ span: 0 }"
                :wrapper-col="{ span: 24 }"
              >
                  ...
                  >
                    <div slot="code" slot-scope="text, record">
                      <a-popover autoAdjustOverflow>
                        ...
                        <span
                          class="can-click overflow-text"
                          @click="goDangerSourceDetailsPage(record)"
                          >{{ text }}</span
                        >
                      </a-popover>
                    </div>
            ...
            <template title="风险管控措施">
              ...
              <a-row>
                <a-col :span="24">
                  <a-form-model-item
                    ref="hardTechnologyControlList"
                    label=" "
                    prop="hardTechnologyControlList"
                    :label-col="{ span: 0 }"
                    :wrapper-col="{ span: 24 }"
                  >
                    <ActiveTable
              ...
              <a-row>
                <a-col :span="24">
                  <a-form-model-item
                    ref="managementControlList"
                    label=" "
                    prop="managementControlList"
                    :label-col="{ span: 0 }"
                    :wrapper-col="{ span: 24 }"
                  >
                    <ActiveTable
              ...
              <a-row>
                <a-col :span="24">
                  <a-form-model-item
                    ref="educationalTrainingControlList"
                    label=" "
                    prop="educationalTrainingControlList"
                    :label-col="{ span: 0 }"
                    :wrapper-col="{ span: 24 }"
                  >
                    <ActiveTable
              ...
              <a-row>
                <a-col :span="24">
                  <a-form-model-item
                    ref="individualProtectionControlList"
                    label=" "
                    prop="individualProtectionControlList"
                    :label-col="{ span: 0 }"
                    :wrapper-col="{ span: 24 }"
                  >
                    <ActiveTable
              ...
              <a-row>
                <a-col :span="24">
                  <a-form-model-item
                    ref="emergencyControlList"
                    label=" "
                    prop="emergencyControlList"
                    :label-col="{ span: 0 }"
                    :wrapper-col="{ span: 24 }"
                  >
                    <ActiveTable
            ...
            <template title="管控责任主体">
              ...
              <a-form-model-item
                ref="riskPointControlResponsibilityList"
                label=" "
                prop="riskPointControlResponsibilityList"
                :label-col="{ span: 0 }"
                :wrapper-col="{ span: 24 }"
              >
                <CommonTable :noPaging="true">
                  ...
                  >
                    ...
                    <div slot="responsibilityPersonId" slot-scope="text, record, index">
                      ...
                      <StaffOrDept
                        ...
                        :checkedTreeNode="
                          iFrom.riskPointControlResponsibilityList[index]
                            .responsibilityPersonId
                            ? iFrom.riskPointControlResponsibilityList[
                                index
                              ].responsibilityPersonId.split(',')
                            : []
                        "
              ...
              <a-row>
                <a-col :span="13">
                  <StaffOrDept
                    ...
                    :labelTitle="'分管安全副总'"
                    :deptTreeId="deptTreeId"
                    :checkedTreeNode="checkedCharge"
                    :checkAbel="false"
                    @getTreeData="getChargeData"
                  ></StaffOrDept>
                </a-col>
              </a-row>
            </template>
            ...
            <FixedBottom>
              <div>
                ...
                <a-button
                  ...
                  class="m-r-15"
                  :loading="loading"
                  @click="saveChemicalIdentSafetyTips"
                  >保存</a-button
                >
              </div>
            </FixedBottom>
    ...
    <CommonDrawer
      title="关联危险源"
      :visible="dangerSourceShow"
      :cancelFn="closeHazardSourceDrawer"
      :width="'90vw'"
    >
      <template>
        <a-tabs>
          <a-tab-pane key="1" tab="共性危险源" forceRender>
            <DangerSourceEqual
              ref="dangerSourceEqual"
              :corporationId="iFrom.corporationId"
              :selectedRowOld="
                (iFrom.dangerSourceList || []).filter(
                  (item) => item.riskDetailedType == 1
                )
              "
            />
          </a-tab-pane>
          <a-tab-pane key="2" tab="特异性危险源" forceRender>
            <DangerSourceSpecial
              ref="dangerSourceSpecial"
              :corporationId="iFrom.corporationId"
              :selectedRowOld="
                (iFrom.dangerSourceList || []).filter(
                  (item) => item.riskDetailedType == 2
                )
              "
            />
          </a-tab-pane>
      ...
      <template slot="btn">
        ...
        <a-button
          ...
          @click="onSubmitHazardSourceDrawer"
          >确定</a-button
        >
      </template>
    </CommonDrawer>
...
import {
  addRiskBookApi,
  getRiskBookDetailApi,
  editRiskBookApi,
  changeRiskBookApi,
  getControlsLevelByRiskLevelApi,
} from "@/services/dangerSource/risk";
...
export default {
  ...
  data() {
    return {
      ...
      iRules: {
        ...
        dangerSourceList: [
          { required: true, message: "关联危险源不能为空", trigger: "change" },
        ],
        hardTechnologyControlList: [
          {
            required: true,
            validator: this.intoFactoryDateValidator,
            trigger: "change",
            text: "工程技术措施",
          },
        ],
        managementControlList: [
          {
            required: true,
            validator: this.intoFactoryDateValidator,
            trigger: "change",
        ...
        educationalTrainingControlList: [
          {
            required: true,
            validator: this.intoFactoryDateValidator,
            trigger: "change",
        ...
        individualProtectionControlList: [
          {
            required: true,
            validator: this.intoFactoryDateValidator,
            trigger: "change",
        ...
        riskPointControlResponsibilityList: [
          {
            required: true,
            validator: this.riskPointControlResponsibilityListValidator,
            trigger: "change",
          },
        ],
        chargeSecurityHeadId: [
          { required: false, message: "分管安全副总不能为空", trigger: "change" },
        ],
      },
      // 主要成分table
      columns: [
  ...
  methods: {
    ...
    getTreeData(value, record, index) {
      ...
      this.$set(
        this.iFrom.riskPointControlResponsibilityList[index],
          ...
          if (res.data && res.data.userId) {
            ...
            this.$set(
              ...
              `${res.data.userName ? res.data.userName : ""}/${
                res.data.adminDeptId ? res.data.adminDeptId : ""
              }`
            );
          } else {
            this.$set(
      ...
      return new Promise((resolve, rej) => {
        getRiskBookDetailApi(apiData)
          .then((res) => {
            let iFrom = res.data;
            console.log(
              iFrom.hardTechnologyControlList,
              "iFrom.hardTechnologyControlListiFrom.hardTechnologyControlList"
            );
            // if()
            iFrom.hardTechnologyControlList = this.addGuid(
              iFrom.hardTechnologyControlList
            );
            iFrom.managementControlList = this.addGuid(iFrom.managementControlList);
            iFrom.educationalTrainingControlList = this.addGuid(
              iFrom.educationalTrainingControlList
            );
            iFrom.individualProtectionControlList = this.addGuid(
              iFrom.individualProtectionControlList
            );
            ...
            this.checkedCharge = this.iFrom.chargeSecurityHeadId
              ? this.iFrom.chargeSecurityHeadId.split(",")
              : [];
            // 部门回显
    ...
    saveChemicalIdentSafetyTips() {
      ...
      apiName(apiData)
        .then((res) => {
          ...
          // 跳转列表页
          this.$router.push({
            path: "/safeManage/dualControlManage/riskManage/riskBook",
          });
        })
        .catch((err) => {})
        .finally(() => {
    ...
    onSubmitHazardSourceDrawer() {
      // 共性
      this.dangerSourceEqualTableData = cloneDeep(
        this.$refs.dangerSourceEqual.selectedRow
      );
      this.dangerSourceEqualTableData.forEach((item) => {
        item.riskDetailedType = 1;
      });
      // 特异性
      this.dangerSourceSpecialTableData = cloneDeep(
        this.$refs.dangerSourceSpecial.selectedRow
      );
      this.dangerSourceSpecialTableData.forEach((item) => {
        item.riskDetailedType = 2;
      });
      let dangerSourceList = [
        ...this.dangerSourceEqualTableData,
        ...this.dangerSourceSpecialTableData,
      ];
    ...
    riskLevelHandleAuto(dangerSourceList) {
      if (dangerSourceList.length) {
        ...
        let riskLevelList = Array.isArray(dictionary("riskLevel"))
          ? dictionary("riskLevel")
          : [];
