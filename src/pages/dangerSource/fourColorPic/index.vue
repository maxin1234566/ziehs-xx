  ...
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        ...
        <a-form-model-item class="flex" label="建筑、楼层" prop="buildingAndFloor">
          <a-cascader
            change-on-select
            v-model="formInline.buildingAndFloor"
            :options="options"
            placeholder="请选择建筑、楼层"
          />
        </a-form-model-item>
    ...
    <DashBtn>
      <div>
        ...
        <a-button type="primary" size="small" @click="toBuildingDataPage"
          >建筑结构数据</a-button
        >
      </div>
    </DashBtn>
    <a-spin :spinning="spinning" wrapperClassName="a-spin">
      <div class="mark-list-container" v-if="markList.length > 0">
        <MarkItem
          ...
          :data="item"
          :key="item.riskFourColorPictureId"
...
export default {
  ...
  methods: {
    ...
    iSearch() {
      // 获取列表
      this.getDataList(this.formInline).finally(() => {
        // this.cancelLoading()
      });
    },
    successCallback() {
    ...
    async getDataList() {
      ...
      if (
        this.formInline.buildingAndFloor &&
        this.formInline.buildingAndFloor.length > 0
      ) {
