    ...
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        ...
        <a-form-model-item label="建筑">
          <a-input
            allowClear
            v-model="formInline.buildingName"
            placeholder="请输入建筑"
          ></a-input>
        </a-form-model-item>
    ...
    >
      ...
      >
        <div slot="corporationId" slot-scope="record">
          <span>{{ getOrgName(record.corporationId) }}</span>
        </div>
        <div slot="action" slot-scope="record">
          <span class="color-0067cc cursor-pointer" @click="showEdit(record)">编辑</span>
          <span class="color-ff4d4f cursor-pointer" @click="deleteItem(record)"
            >删除</span
          >
        </div>
      </a-table>
    ...
    <CommonModal :title="title" :visible="visible" :cancelFn="cancelFn">
      <template slot="form">
        ...
        >
          ...
          <NewCommonSearchItem
            :CommonFormInline="modelForm"
            :rules="rules"
            :notTablePage="true"
          ></NewCommonSearchItem>
          <a-form-model-item class="flex" label="建筑" prop="buildingName">
            <a-input
              v-model="modelForm.buildingName"
              placeholder="请输入建筑"
              :maxLength="30"
            ></a-input>
          </a-form-model-item>
...
import {
  ...
  riskBuildingUpdate,
  riskBuildingDelete,
  getRiskBuildingTreeData,
} from "@/services/dangerSource/fourColor";
...
export default {
  ...
  data() {
    return {
      ...
      modelForm: {},
      rules: {
        buildingName: [{ required: true, message: "请输入建筑", trigger: "blur" }],
  ...
  methods: {
    getOrgName(str) {
      let list = JSON.parse(sessionStorage.getItem("allOrgList")) || [];
      list = list.filter((item) => {
        return item.orgId == str;
      });
      if (list.length) {
