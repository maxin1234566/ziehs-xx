  ...
  <div class="searchtable-wrapper clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
        ...
        <div class="head-module">
          <div v-if="isCheck">
            <div class="head-l-module">
              <span class="is-check-person">检查人：<img v-if="imageUrl" :src="imageUrl" class="signImg" />
              </span><span>检查时间:{{ baseInfoData.checkTime }}</span>
            </div>
            <div class="head-r-module">
              <span>待排查: {{ baseInfoData.unUseCheckCount }}</span><span>应排查: {{ baseInfoData.totalCheckCount || "--"
              }}</span><span>排查进度:
          ...
          <div v-else>
            <div class="head-l-module">
              <span class="no-check-person" style="width: 260px;">
                <span style="color: red;">*</span>
                检查人：
                <UploadBtnStyle :accept="['.png', '.jpg', '.JPGE']" :maxSize="10" :limit="1"
                  @handleSuccess="handleSuccessRefFile" :showUploadList="false" :useSlot="true" :fileLists="fileList">
                  <img v-if="imageUrl" :src="imageUrl" alt="avatar" class="signImg" />
                  <div v-else>
                    <a-button>
                      <a-icon type="upload" />
                      上传签名
                ...
                <a-button v-if="imageUrl" type="link" size="small" class="left-margin" @click="hadleDeleteFile" ghost>
                  删除
                </a-button>
              </span>
              <span style="width: 330px">
                <span style="color: red;">*</span>
                检查时间：
                <a-date-picker v-model="baseInfoData.checkTime" />
              </span>
            </div>
            <!-- <div class="head-r-module">
                                          <span>待排查: xxxxx</span
        ...
        <div class="stak-content">
          <div class="stak-content-title">
            <a-row>
              ...
              <a-col class="stak-col" :span="4">
                <div class="gutter-title">排查结果</div>
              </a-col>
              <a-col class="stak-col" :span="3">
          ...
          <div class="stak-content-text" v-for="(itemMeasure, index) in measureList" :key="index">
            <div v-if="isCheck" :class="{
            ...
            <div v-else :class="{
              ...
              <a-row type="flex" justify="space-around" align="middle">
                ...
                <a-col class="stak-col" :span="3">
                  <div v-for="(item, index) in itemTask.list" :key="index">
                    <p v-if="item.type == itemMeasure.type">
                      <a-textarea size="small" placeholder="请输入" v-model="item.checkStandard" />
                    </p>
                  </div>
                </a-col>
                <a-col class="stak-col" :span="3">
                  <div v-for="(item, index) in itemTask.list" :key="index">
                    <p v-if="item.type == itemMeasure.type">
                      <a-textarea size="small" placeholder="请输入" v-model="item.checkResult" />
                    </p>
                  </div>
                </a-col>
                ...
                <a-col class="stak-col" :span="3">
                  <div v-for="(item, index) in itemTask.list" :key="index">
                    <p v-if="item.type == itemMeasure.type">
                      ...
                      ]" v-model="item.checkStatus" @change="handleSeleckChange(item)" />
                    </p>
                  </div>
                </a-col>
                <a-col class="stak-col" :span="3">
                  <div v-for="(item, index) in itemTask.list" :key="index">
                    <p v-if="item.type == itemMeasure.type">
                      <a-textarea size="small" placeholder="请输入" v-model="item.abnormalText" />
                    </p>
                  </div>
                </a-col>
                <a-col class="stak-col" :span="3">
    ...
    <div class="floor">
      <div>
        ...
        <span></span>
        <a-button v-if="isCheck" type="primary" @click="handleDownloadPdf">下 载</a-button>
        <a-button v-else type="primary" @click="handleConfirm">确 定</a-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getStr(obj){
      if(obj.riskPoint&&obj.riskPoint.dangerSourceEventList&&obj.riskPoint.dangerSourceEventList.length){
   const  textArr = obj.riskPoint.dangerSourceEventList ? obj.riskPoint.dangerSourceEventList : [];
            let text = "";
    ...
    handleConfirm() {
      ...
      // console.log('我要确定了我要确定了')
      if(!this.baseInfoData.checkTime) return this.$antMessage.error('请选择检查时间后再进行提交');
      if(!this.baseInfoData.handSignature) return  this.$antMessage.error('请上传签名后再进行提交');
