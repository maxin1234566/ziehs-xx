  ...
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    ...
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        <NewCommonSearchItem ref="commonSearchItem" :CommonFormInline="formInline" @corporationChange="corporationSearchChange"></NewCommonSearchItem>
        <a-form-model-item label="课件/文件名">
          <a-input v-model="formInline.name" placeholder="请输入课件名称/文件名称" allowClear></a-input>
        </a-form-model-item>
        <a-form-model-item label="类型">
          <a-select v-model="formInline.type" placeholder="请选择类型" allowClear>
      ...
      <template slot="form">
        <a-form-model :label-col="{ style: { width: '80px' } }" :wrapper-col="{ style: { width: 'calc(100% - 80px)' } }" :colon="false" labelAlign="left">
          ...
          <a-form-model-item class="flex modal-form-text" label="所属科目">
            <div>{{findText(subjectListType, "subjectId", currentMsg.subjectId).name}}</div>
          </a-form-model-item>
...
import { GetCoursewarelist, InsertCourseware, UpdateCourseware, DeleteCourseware, GetSubjectlist, DownLoadCourseware,DownLoadFiles } from "@/services/api.js";
...
export default {
  ...
  data() {
    return {
      tableSpinning:false,
      addLoading: false,
      actions: `${process.env.VUE_APP_API_BASE_URL}/ziehs-sysManage/api/file/uploadFile`,
      editLoading: false,
      newFile: {
        id: undefined,
        url: undefined,
  ...
  methods: {
    ...
    downLoad(record) {
      DownLoadFiles({id:record.fileId}).then((res)=>{
        // const blob = new Blob([res]);
	      // const objectURL = URL.createObjectURL(blob);
	      // let btn = document.createElement('a');
	      // btn.download = record.name;
	      // btn.href = objectURL;
	      // btn.click();
