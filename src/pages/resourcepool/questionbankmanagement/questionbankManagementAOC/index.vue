      ...
      >
        <a-row>
          <a-col :span="12">
            ...
            >
              <a-select
                v-model="policyForm.topicType"
                placeholder="请选择类型"
                @change="topicTypeSelect"
              >
                <a-select-option
                  v-for="policy of policyLowType"
                  :value="policy.key"
                  :key="policy.id"
                  >{{ policy.value }}</a-select-option
                >
              </a-select>
            </a-form-model-item>
            <a-form-model-item
        ...
        <a-row>
          <a-col :span="24">
            <a-form-model-item
              label="题干"
              prop="topicTitle"
              :label-col="labelColEditor"
              :wrapper-col="wrapperColEditor"
            >
              <Editor
                :height="height"
                v-model="policyForm.topicTitle"
                :isClear="isClear"
          ...
          <a-col :span="24">
            ...
            >
              <div class="input_check_div" v-show="policyForm.optionsRels.length">
                <div
                  class="item_input_check_div"
                  v-for="(item, index) in policyForm.optionsRels"
                  :key="index"
                >
                  ...
                  <a-radio
                    ...
                    v-show="policyForm.topicType == 1 || policyForm.topicType == 3"
                    >设为正确答案</a-radio
                  >
                  <span
                    class="color-ff4d4f cursor-pointer del_span"
                    @click="delAnswer(item)"
                    >删除</span
                  >
                </div>
              </div>
              <a-button type="primary" @click="addChoose" :disabled="addDisabledBtn"
                >添加选项</a-button
              >
            </a-form-model-item>
          </a-col>
        ...
        <a-row>
          <a-col :span="24">
            ...
            >
              <Editor
                :height="height"
                v-model="policyForm.topicSolution"
                :isClear="isClear"
                @submitEditor="submitEditorTwo"
              ></Editor>
            </a-form-model-item>
          </a-col>
        </a-row>
...
export default {
  ...
  data() {
     return {
      ...
      policyRules: {
        topicType: [{ required: true, message: "类型不能为空", trigger: "change" }],
        sujectId: [{ required: true, message: "题目分类不能为空", trigger: "change" }],
        topicTitle: [{ required: true, message: "题干不能为空", trigger: "blur" }],
        topicScore: [
          {
            required: true,
        ...
        optionsRels: [{ required: true, message: "答案选项不能为空", trigger: "change" }],
      },
      type: undefined,
      topicId: undefined,
      policyLowType: [
  ...
  methods: {
    treeChange(id, name) {
      if (name.length) {
        this.policyForm.deptName = name[0];
      } else {
        this.policyForm.deptName = "";
      }
      // console.log(id, name);
    },
    // 获取组织下所有部门
      ...
      DetaillDataList({ topicId: this.topicId })
        .then((res) => {
          ...
          this.$refs.commonSearchItem.corporationChange(
            this.policyForm.corporationId,
            this.policyForm.deptId
          );
    ...
    policySubmit: _.throttle(function () {
      ...
      let api = this.type == "change" ? AddOrChangeDataListupdate : AddOrChangeDataList;
      api(params)
        .then((res) => {
          if (res.code == 20000) {
    ...
    checkSolution() {
      ...
      } else if (this.policyForm.topicType == "1" || this.policyForm.topicType == "3") {
  ...
  computed: {
    ...
    addDisabledBtn() {
      ...
      } else if (this.policyForm.topicType == 3) {
        // 判断
        if (this.policyForm.optionsRels.length == 2) {
          return true;
        }
