  ...
  <div class="searchtable-wrapper clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
    ...
    <SearchTerm>
      <a-form-model layout="inline" :model="formInline" :colon="false">
        <CommonDept
          ...
          :hasDepartment="true"
          @corporationChange="corporationSearchChange"
        ...
        <a-form-model-item label="题干">
          <a-input
            v-model="formInline.topicTitle"
            placeholder="支持题干模糊搜索"
            allowClear
          ></a-input>
        </a-form-model-item>
      ...
      >
        ...
        <div slot="action" slot-scope="record">
          <span
            ...
            @click="jumpAddOrDetail('change', record)"
            >编辑</span
          >
          <!-- <span class="color-0067cc cursor-pointer" @click="copy(record)"
            >复制</span
          >-->
          <span class="color-ff4d4f cursor-pointer" @click="delDataList(record)"
            >删除</span
          >
        </div>
      </a-table>
    ...
    <CommonModal :title="dictTitle" :visible="visible" :cancelFn="cancelFn">
      <template slot="form">
        ...
        >
          <a-form-model-item label="第一步">
            ...
            <div class="addSuccess cursor-pointer" @click="downloadTemplate">
              下载模板
            </div>
          </a-form-model-item>
          ...
          <a-form-model-item label="第三步" prop="file">
            ...
            <a-select
              v-model="dictForm.corporationId"
              placeholder="请选择组织"
              @change="corporationChange"
            >
              <a-select-option
                v-for="item of getCommonAddOrgnizeList"
                :key="item.orgId"
                :value="item.orgId"
      ...
      <template slot="btn">
        ...
        <a-button class="m-l-15" type="primary" :loading="dictLoading" @click="submitDict"
          >确定</a-button
        >
      </template>
    </CommonModal>
  ...
  methods: {
    ...
    corporationChange(corporationId, deptId) {
      ...
      this.$set(
        this.dictForm,
        "centerId",
        this.getMappingValue(this.getCommonAddOrgnizeList, "orgId", corporationId)
          .centerId
      );
      this.sujectList = [];
      if (corporationId) {
    ...
    downloadTemplate() {
      DownLoadFiles({ id: "1822113664598949888" }).then((res) => {
        const blob = new Blob([res.data]);
        const objectURL = URL.createObjectURL(blob);
        let btn = document.createElement("a");
        btn.download = "试题导入模板.xlsx";
        btn.href = objectURL;
        btn.click();
    ...
    submitDict() {
      ...
      post(`/ziehs-integration/api/course/topic/upload`, fromData)
        .then((res) => {
          this.getTabList();
