...
export default {
  ...
  data() {
    return {
      ...
      corporationId: JSON.parse(sessionStorage.getItem("examinaObj_xt")).firstStep.corporationId,
  ...
  methods: {
    ...
    subjectsDataList() {
      let sessObj = JSON.parse(sessionStorage.getItem("examinaObj_xt"));
      SubjectsDataList({
        corporationId: sessObj.firstStep.corporationId
      })
        .then((res) => {
    ...
    getKDataList(list) {
      ...
      // console.log(this.list,'ddd获取手动选题回显d');
    },
    ...
    //确认批量设置分值弹框
    submitAllSetVisible() {
      console.log('this.list点击确认的list',this.list);
