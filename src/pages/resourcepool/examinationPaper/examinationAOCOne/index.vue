<template>
  <HasFixedBottomWrapper>
    <div class="arc">
      <!-- 步骤条 -->
      <div class="steps">
        <span class="active">1/设置设卷信息234</span>
        <span>2/设置试卷题目</span>
      </div>
    ...
    <div slot="fixedBottom">
      <FixedBottom>
        ...
        <a-button type="primary" :style="{ marginLeft: '20px' }" @click="submit"
          >下一步</a-button
        >
      </FixedBottom>
    </div>
...
import { SubjectsDataList, LookQuestionlDataList } from "@/services/questionmodel.js";
...
export default {
        ...
        name: [
          {
            ...
            minLength: 2,
            text: "试卷名称",
            validator: formValidator.limitLength,
            trigger: "blur",
          },
        ],
  ...
  created() {
    ...
    if (!isJump) {
      this.$router.replace("/ehsGerneralManage/educationmanagement/examinationPaper");
  ...
  methods: {
    ...
    detailList() {
      ...
      LookQuestionlDataList({ paperId: this.paperId }).then(
        // productId
        (res) => {
          this.$refs.commonSearchItem.corporationChange(res.data.corporationId);
          this.policyForm = {
            name: res.data.name,
    ...
    submit() {
      ...
      this.$router.replace("/ehsGerneralManage/educationmanagement/examinationAOCTwo");
    },
    //返回
    cancel() {
