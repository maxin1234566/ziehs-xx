<template>
  <HasFixedBottomWrapper>
    <div class="common-search">
      <NewCommonSearchItem
        :CommonFormInline="commonFormInline"
        :notTablePage="true"
        :isNgForm="true"
        :config="formConfig"
        :labelCol="{ style: { width: formConfig.labelWidth + 'px' } }"
    ...
    <CommonModal title="办理" :visible="chooseStaffVisible" :cancelFn="chooseStaffCancle">
      ...
      <template slot="btn">
        <a-button type="primary" @click="chooseStaffConfirm" :loading="modalLoading"
          >发送</a-button
        >
    ...
    <CommonModal title="办理" :visible="rebackVisable" :cancelFn="nextCancle">
      ...
      <template slot="btn">
        <a-button type="primary" @click="nextConfirm" :loading="modalLoading"
          >下一步</a-button
        >
      ...
      <FixedBottom v-if="formConfig.isHiddenTab != 2">
        <div>
          <a-button
            class="m-r-15"
            :disabled="btnStatus"
            @click="buttonTwoTextFn"
            v-if="buttonTwoVisable"
            >{{ this.buttonTwoText }}</a-button
          >
          <a-button
            class="m-r-15"
            @click="dataFormSubmit(6)"
            v-if="isShowBtn3"
          ...
          <a-button
            type="primary"
            :disabled="btnStatus"
            @click="buttonOneTextFn"
            v-if="!preview"
            :loading="submitLoading"
            >{{ this.buttonOneText }}</a-button
          >
        </div>
      </FixedBottom>
      <FixedBottom v-else>
...
import {
  InsertFormData,
  UpdateFormData,
  InsertFormDataByEnter,
  UpdateFormDataByEnter,
  GetId,
  GetFirstNode,
  GetNextTask,
  SaveBusinessProcess,
...
export default {
  ...
  created() {
    this.routeParams = this.$route.params.formData
      ? this.$route.params
    ...
    this.companyId = this.customerCompanyId
  ...
  methods: {
    async init() {
      ...
      this.$nextTick(() => {
        for (let i = 0; i < arr.length; i++) {
          if (
            arr[i].type == "batch" &&
            !arr[i].options.hidden &&
            arr[i].options.disabled
          ) {
      ...
      this.buttonOneText = result11.data.buttonOneText
        ? result11.data.buttonOneText
        : buttonOneText;
      this.buttonTwoText = result11.data.buttonTwoText
        ? result11.data.buttonTwoText
        : buttonTwoText;
    ...
    dataFormSubmit: debounce(
      function (flag) {
        // 1:审核驳回  2:保存-到草稿箱  3:撤回   4:退回  5:只涉及表单不涉及流程的业务 6：只是暂存表单数据，不往下走流程   else就是审核通过
            ...
            this.$refs.formbuild.getData().then(async (obj) => {
              ...
              try {
                if (this.isEdit) {
                  ...
                  if (
                    this.blackId &&
                    this.$route.path == "/cooperationPartner/addhiddendanger10"
                  ) {
                    // 是黑名单的新建页面
                    updateEnterBlack({ blackId: this.blackId, blackStatus: 1 })
                      .then(() => {})
                      .catch((err) => {
                        console.log(err);
                      });
    ...
    async chooseStaffConfirm() {
      if (
        (this.staffArr.length > 0 && this.nodeId.split("_")[0] != "endEvent") ||
        this.nodeId.split("_")[0] == "endEvent"
      ) {
        let params6 = {
          taskId: this.taskId,
          userId: this.userInfo.userId,
    ...
    async updateFormDataFn(num) {
      try {
        ...
        params9.updateMap = this.newFormObj.updateMap
          ? this.newFormObj.updateMap
          : this.newFormObj.formData;
