<template>
  <HasFixedBottomWrapper>
    ...
    <div slot="fixedBottom">
      <FixedBottom>
        <div>
          <a-button class="m-r-15" @click="dataFormSubmit()" v-if="rebackBtnStatus"
            >撤回</a-button
          >
...
export default {
  ...
  async created() {
    ...
    let tempId = getQueryVariable("formId")
      ? getQueryVariable("formId")
      : this.$route.query.formId;
    let businessId = getQueryVariable("businessId")
      ? getQueryVariable("businessId")
      : this.$route.query.businessId;
    let url = getQueryVariable("url") ? getQueryVariable("url") : this.$route.query.url;
    let draftUrl = getQueryVariable("draftUrl")
      ? getQueryVariable("draftUrl")
      : this.$route.query.draftUrl;
  ...
  methods: {
    ...
    async init(obj) {
      ...
      function getModelArr(arr) {
        for (let i = 0; i < arr.length; i++) {
          if (
            formData.templateData.list[i].rules &&
            formData.templateData.list[i].rules[0]
          ) {
      ...
      this.$nextTick(() => {
        for (let i = 0; i < arr.length; i++) {
          if (
            arr[i].type == "batch" &&
            arr[i].model &&
            document.getElementById(arr[i].model)
          ) {
    ...
    // 表单提交
    dataFormSubmit() {
      // 1:审核驳回  2:保存-到草稿箱  3:撤回   4:退回  5:只涉及表单不涉及流程的业务   else就是审核通过
