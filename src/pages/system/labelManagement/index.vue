  ...
  <div class="role-console-box clx-show-scroll clx-flex-1 beauty-scroll">
    ...
    >
      <template>
        ...
        <a-table bordered :data-source="labelDataSource" :columns="labelColumns" :rowKey="(record,index)=>{return record.timestamp? record.timestamp: record.labelItemId}" >
          <template slot="labelItemName" slot-scope="text, record">
            <editable-cell :editable="true" :text="text" @change="onCellNameChange(record, 'labelItemName', $event)" />
          </template>
          <template slot="labelItemCode" slot-scope="text, record">
            <editable-cell :editable="record.labelItemId == ''" :text="text" @change="onCellCodeChange(record, 'labelItemCode', $event)" />
          </template>
          <template slot="labelItemDescription" slot-scope="text, record">
            <editable-cell :editable="true" :text="text" @change="onCellDesChange(record, 'labelItemDescription', $event)" />
          </template>
...
export default {
  ...
  methods: {
    ...
    async manageLabel(item){
      console.log('SCC',this.labelDataSource);
      this.labelId = item.labelId;
    ...
    onCellNameChange(key, dataIndex, value) {
      ...
      const target1 = labelDataSource.find(item => item.labelItemId != '' && (item.labelItemId == key.labelItemId));
      const target2 = labelDataSource.find(item => item.timestamp == key.timestamp);
      if(target1 != undefined){
        target1[dataIndex] = value;
        return
      }
      if(target1 == undefined){
        target2[dataIndex] = value;
        return
      }
