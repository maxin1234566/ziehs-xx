      ...
      <div slot="form">
        <div class="item_fl">
          <a-form-model
            ...
            :colon="false"
          >
            <a-form-model-item ref="deptCode" label="部门编码" prop="deptCode">
              <a-input
                :disabled="disabled"
                :maxLength="10"
                v-model.trim="formDepartment.deptCode"
                placeholder="请输入部门编码"
              />
            </a-form-model-item>
            <a-form-model-item ref="deptName" label="部门名称" prop="deptName">
              <a-input
                :maxLength="nameMaxLength"
                v-model.trim="formDepartment.deptName"
                placeholder="请输入部门名称"
                disabled
              />
            </a-form-model-item>
             <a-form-model-item ref="bossNameOne" label="负责人" prop="bossNameOne">
              <a-input
                :maxLength="nameMaxLength"
                v-model.trim="formDepartment.bossNameOne"
             ...
             <staffOrDept 
              ...
              />
              <a-form-model-item ref="parentName" label="上级部门" prop="parentName">
              <a-input
                :maxLength="nameMaxLength"
                v-model.trim="formDepartment.parentName"
                placeholder="请输入上级部门"
                disabled
              />
            </a-form-model-item>
            ...
            <!-- <a-form-model-item ref="bossIdOne" label="负责人" prop="bossIdOne"> -->
              <!-- <a-select
                v-model="formDepartment.bossIdOne"
                placeholder="请选择负责人"
                show-search
...
export default {
  ...
  methods: {
        ...
        // this.getAllUserByCompanyIdFn()
          // .then((res) => {
            ...
            // GetDepartCode().then((res) => {
              // if (res.data) {
                // this.formDepartment.deptCode = res.data;
                this.btnType = 1;
                this.sendParentId = [];
                this.sendBossId = [];
    ...
    getRoleItemDetailsFn(apiData) {
      return getDepartmentDetails(apiData)
        .then((res) => {
          ...
          this.sendBossId = [this.formDepartment.bossIdTwo]
          // this.sendSecondBossId = [this.formDepartment.bossIdTwo]
        })
        .catch((err) => {});
    },
    // 修改
    ...
    getUserTreeData(data) {
      if(data.treeNameAndCodeList.length > 0) {
        this.formDepartment.bossIdOne = data.treeNameAndCodeList[0].id;
        this.formDepartment.bossIdTwo = data.treeNameAndCodeList[0].id;
      }
      if (!formValidator.formItemValidate(this, "bossIdOne", "ruleForm")) {
        return;
      }
    },
  },
