    ...
    <div class="app-auth-rolemain">
      ...
      <div class="app-auth-rolemain-left">
        <div class="left-title">
          <span class="left-title-text" :title="selAppAuthItem.productName">{{
            selAppAuthItem.productName
          }}</span>
        ...
        <ul class="app-auth-rolemain-left-list">
          <li
            :class="item.active ? 'left-list-item-active' : ''"
            v-for="item in appAuthRoleList"
            :key="item.roleId"
            @click="selAppAuthRoleListItem(item)"
          >
            <div class="rule-name">{{ item.roleName }}</div>
            <a-popover
              v-model="item.popoverShow"
              placement="leftTop"
              overlayClassName="rule-name-btn-tip-box"
            >
      ...
      <div class="app-auth-rolemain-right">
        ...
        <SearchTerm>
          <a-form-model layout="inline" :model="formInline" :colon="false">
            <a-form-model-item label="姓名">
              <a-input
                class="app-auth-rolesearch-left-item"
        ...
        <div class="app-auth-roleapply clx-btn-dashed-box">
          <a-button type="dashed" @click="openGrantModel" class="clx-btn-margin-right"
            >授权用户</a-button
          >
        </div>
        <!-- 表格 -->
...
import {
  getAppAuthRoleList,
  getGrantTreeList,
  getAppAuthRoleUserList,
  getAppAuthRoleDetail,
  rmAppAuthRole,
  getGrantModelTreeData,
  rmGrantUser,
} from "@/services/api";
...
export default {
  ...
  mixins: [treeMixin, fromMaxLength, teableCenterEllipsis, teableSelected, cancelLoading],
  data() {
    return {
      // 角色弹窗model-显示隐藏
      roleModelShow: false,
  ...
  created() {
    ...
    this.selAppAuthItem =
      console_appAuthRole && console_appAuthRole.appAuthRole
        ? console_appAuthRole.appAuthRole
        : this.selAppAuthItem;
    let zconsole_userInfo = JSON.parse(sessionStorage.getItem("zconsole_userInfo"));
    this.companyId =
      zconsole_userInfo &&
      zconsole_userInfo.company &&
      zconsole_userInfo.company.companyId
        ? zconsole_userInfo.company.companyId
        : undefined;
    // 获取角色列表
  ...
  methods: {
    showPhoneClick(item, flag) {
      // if (!this.canClickBtnMixin("role_phone")) {
      //   return;
      // }
    ...
    addRole() {
      // 获取当前产品的菜单权限树
      this.getGrantModelTree().then((res) => {
        this.roleModelData = {
          companyId: this.companyId,
          productId: this.selAppAuthItem.productId,
    ...
    changeRole: debounce(
      function (item) {
        ...
        getAppAuthRoleDetail(apiData).then((res) => {
          let resourceIdList = clxTools.recursionArr(
            res.data.resourceTreeList,
            "resourceId"
          );
