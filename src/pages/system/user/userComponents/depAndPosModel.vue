    ...
    >
      <a-spin :spinning="spinning" wrapperClassName="a-spin">
        <div class="filter-form-box">
          ...
          <a-form-model
            v-for="(item, index) in userDeptRelsList"
            :key="item.guid"
            ref="filterForm"
            class="filter-form"
            :model="item"
          ...
          >
            <a-form-model-item
              ref="deptId"
              prop="deptId"
              class="filter-form-item filter-form-left"
            >
              <a-tree-select
                v-model="item.deptId"
                :dropdown-style="{
            ...
            <a-form-model-item
              ref="positionId"
              prop="positionId"
              class="filter-form-item filter-form-right"
            >
              <a-select
                v-model="item.positionId"
                allow-clear
                placeholder="请选择职位"
                @change="positionIdChange($event, item)"
              >
                <a-select-option
            ...
            <div class="filter-form-btn-box">
              <img
                ...
                @click="addUserDeptRels"
                v-if="index == userDeptRelsList.length - 1"
                src="@/assets/depAndPosModel/add-icon.svg"
              />
              <img
                class="btn-icon btn-rm"
                v-if="userDeptRelsList.length > 1"
                @click="rmUserDeptRels(item)"
                src="@/assets/depAndPosModel/rm-icon.svg"
              />
              <div class="check-btn-box" @click="changeAdminDept(item)">
                <img
                  class="btn-icon check-btn"
                  v-if="item.adminDept == 1"
                  src="@/assets/depAndPosModel/check-icon-active.svg"
                />
                <img
                  class="btn-icon check-btn"
                  v-else
                  src="@/assets/depAndPosModel/check-icon.svg"
                />
      ...
      <template slot="btn">
        ...
        <a-button class="m-l-15" type="primary" :loading="loading" @click="submitBtn"
          >确定</a-button
        >
      </template>
    </CommonModal>
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      rulesAuth: {
        deptId: [{ required: false, message: `部门不能为空`, trigger: "change" }],
        positionId: [{ required: false, message: `职位不能为空`, trigger: "change" }],
      },
      // 条件列表
      positionList: [],
  ...
  methods: {
    ...
    openDepAndPosModel() {
      ...
      return UserDetailInterFace(apiData)
        .then((res) => {
          ...
          dataObj.deptAndPositionList = dataObj.deptAndPositionList
            ? dataObj.deptAndPositionList
            : [];
          // 添加id唯一标识
          dataObj.deptAndPositionList.forEach((item) => {
            item.guid = this.guid();
          });
          this.userDeptRelsList = dataObj.deptAndPositionList.length
            ? dataObj.deptAndPositionList
            : cloneDeep(this.userDeptRelsListInit);
    ...
    submitBtn() {
      ...
      apiData.userDeptRels.forEach((item) => {
        if (item.adminDept != "ture" || item.adminDept != "false") {
          item.adminDept == 1 ? (item.adminDept = true) : (item.adminDept = false);
        }
      });
