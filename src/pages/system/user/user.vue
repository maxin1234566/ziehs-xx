<template>
  ...
  <div class="user-box clx-show-scroll clx-flex-1 beauty-scroll">
    ...
    <div class="user-main">
      ...
      <div class="user-main-left bea_scroll_xt">
        ...
        <a-tree
          ...
          :tree-data="treeData"
          @select="selectTree"
          :selectedKeys.sync="selectedKeysOld"
          :defaultExpandAll="false"
          :expanded-keys="expandedKeys"
          :auto-expand-parent="userTreeExpand"
          @expand="onExpand"
        />
      </div>
        ...
        >
          ...
          >
            <div slot="phone" slot-scope="text, record">
              <!-- <a-popover autoAdjustOverflow v-if="!record.show"> -->
              ...
              <img
                ...
                class="imgShow"
                v-if="!record.show"
                @click="showPhoneClick(record, true)"
              />
              <span @click="showPhoneClick(record, false)" v-if="record.show">
                {{ text || "--" }}
              </span>
            </div>
            ...
            <div slot="action" slot-scope="text, record">
              <span
                class="color-0067cc table-btn cursor-pointer"
                :class="btnAuthDisable(record) ? 'table-btn-back-disable' : ''"
                @click="editDict(record)"
                >编辑</span
              >
              <span
                class="color-0067cc table-btn cursor-pointer"
                @click="actionShare(record)"
                >业务组织</span
              >
              <span
                class="color-0067cc table-btn cursor-pointer"
                @click="openDepAndPosModel(record)"
                >部门及职位</span
              >
              <span
                class="color-0067cc table-btn cursor-pointer"
                @click="openLabelModel(record)"
                >标签</span
              >
            </div>
          </a-table>
    ...
    <CommonModal :title="dictTitle" :visible="addUserFlag" :cancelFn="cancelThing">
      <div slot="form">
        ...
        >
          ...
          <a-form-model-item label="姓名" prop="userName">
            <a-input
              ...
              :disabled="addOrChangeUser == 'edit' ? true : false"
              placeholder="请输入姓名"
              :maxLength="14"
            />
          </a-form-model-item>
          ...
          <a-form-model-item label="工号" prop="jobNumber">
            <a-input
              v-model.trim="addUserForm.jobNumber"
              placeholder="请输入工号"
              :maxLength="jobNumberMaxLength"
              disabled
            />
        ...
        <div v-show="formCustomNew.length">
          ...
          >
            <a-form-model-item
              v-for="(item, index) in formCustomNew"
              :key="index"
              :label="item.label"
              :prop="item.prop"
            >
              <a-input
                v-model.trim="addUserFormT[item.prop]"
      ...
      <div slot="btn">
        ...
        <a-button
          ...
          @click="submitAddModel('continue')"
          v-show="addOrChangeUser != 'edit'"
          >提交并继续添加</a-button
        >
      </div>
    </CommonModal>
    ...
    <CommonModal
      title="申请人列表"
      :visible="FlagApplicant"
      :cancelFn="cancelThingApplicant"
    >
      <div class="btns_div clx-btn-dashed-box">
        <a-button
          class="clx-btn-margin-right"
          type="dashed"
          @click="activateOrRemove('activate', 'all')"
          >激活</a-button
        >
        <a-button
          class="clx-btn-margin-right"
          type="dashed"
          @click="activateOrRemove('remove', 'all')"
          >移除</a-button
        >
      </div>
      <CommonTable
        :page="pageApplicant"
        :pageNoChange="pageNoChangeApplicant"
        :showSizeChange="showSizeChangeApplicant"
      >
        <a-table
          :row-selection="{
        ...
        >
          ...
          <div slot="actionApplicant" slot-scope="text, record">
            <span
              ...
              @click="activateOrRemove('activate', 'one', record)"
              style="margin-right: 10px"
              >激活</span
            >
            <span
              class="color-0067cc cursor-pointer"
              @click="activateOrRemove('remove', 'one', record)"
              >移除</span
            >
          </div>
        </a-table>
    ...
    <!-- 邀请用户加入企业组件 -->
    <JoinEnterprise
      :JoinQyFlag="JoinQyFlag"
      :step="false"
      :addcodeObj="addcodeObj"
      @closeJoinEnterpriseFlag="closeJoinEnterpriseFlag"
      @toRegenerateCode="toRegenerateCode"
      @joinUser="joinUser"
    />
    ...
    <SetAuth
      ...
      :userRoleList="treeDataRoles"
      :selItem="selItem"
      @closeAuthModel="cancelAuthModel"
    />

    <!-- 字段管理 -->
    <SelTable
      v-model="selectModel"
      :tableColumnAll="tableColumnAllList"
      :tableColumnIng="seltableColumnList"
      @setTableColumn="setTableColumn"
      :seltableColumnVersion="seltableColumnVersion"
    />

    <!-- 部门及职位弹窗 -->
    <DepAndPosModel
      v-model="depAndPosModelShow"
      :depAndPosModelData="depAndPosModelData"
      :treeDataG="treeDataG"
    ...
    <!-- 标签弹窗 -->
    <LabelModel
      v-model="labelModelShow"
      :labelModelData="labelModelData"
      @getTableList="getTableList"
    />

    <!-- 业务组织弹窗 -->
    <CommonModal title="业务组织" :visible="shareVisible" :cancelFn="shareCancle">
      <a-checkbox @change="onCheckboxChange" :checked="allCheck"> 全选 </a-checkbox>
      <a-tree
        v-model="divideCheckedKeys"
        :selected-keys="selectedBusinessKeys"
        checkable
        :tree-data="divideTreeData"
        checkStrictly
        :replaceFields="{
          children: 'childList',
          title: 'orgName',
...
export default {
    ...
    return {
      ...
      columnsApplicant: [
        ...
        },
        {
          ...
          align: "center",
          customRender: (text) => {
            return <div>{getDictionaryItemObj("companyUserInfoStatus", text)}</div>;
          },
        },
        {
          dataIndex: "action",
  ...
  watch: {
    divideCheckedKeys(newVal) {
      if (
        newVal.length == this.dataFlatTreeData.length ||
        (newVal.checked && newVal.checked.length == this.dataFlatTreeData.length)
      ) {
        this.allCheck = true;
      } else {
        this.allCheck = false;
  ...
  computed: {
    ...
    // 公司信息
    company() {
      let company = JSON.parse(sessionStorage.getItem("zconsole_userInfo")).company
  ...
  methods: {
    // 扁平化数组
    dataFlat(params) {
      // console.log(params, "paramsparamsparams", "我走了");
      let list = [];
      params.forEach((item) => {
        // console.log(99999999);
        item.childList && item.childList.length
          ? (list = [...list, item, ...this.dataFlat(item.childList)])
          : (list = [...list, item]);
      });
    ...
    onCheckboxChange(e) {
      ...
      if (e.target.checked) {
        this.divideCheckedKeys = this.dataFlatTreeData;
      } else {
    ...
    async actionShare(record) {
      ...
      getCorporationTree()
        .then((res) => {
          console.log(res.data, "业务组织树 我要处理业务组织树");
          this.divideTreeData = [res.data] || [];
          this.dataFlatTreeData = this.dataFlat([res.data]).map((item) => item.orgId);
          if (this.dataFlatTreeData.length == this.divideCheckedKeys.length) {
            this.allCheck = true;
          } else {
            this.allCheck = false;
          }
    ...
    shareConfirm() {
      // console.log("this.divideCheckedKeys", this.divideCheckedKeys);
      // return;
      let para = {
        userId: this.currentMsg.userId,
        orgIdList: this.divideCheckedKeys.checked
          ? this.divideCheckedKeys.checked
          : this.divideCheckedKeys,
      };

      divideCorporation(para)
        .then((res) => {
          this.$antMessage.success("分配成功！");
          if (
            this.currentMsg.userId ==
            JSON.parse(sessionStorage.getItem("zconsole_userInfo")).user.userId
          ) {
            // 如果是自己给自己分配调用当前登录人接口
    ...
    setColumnsDataFormLoc() {
      let seltableColumnListArr =
        JSON.parse(localStorage.getItem("console_user_seltableColumnList")) || [];
      if (seltableColumnListArr.length) {
        ...
        if (
          selectedRowKeysArr.length &&
          selectedRowKeysArr[0].version == this.seltableColumnVersion
        ) {
          // 根据筛选的显示对应表头
    ...
    //用户编辑事件
    editDict(record) {
      if (!this.canClickBtnMixin("user_edit")) {
        return;
      }
      ...
      UserDetailInterFace({ userId: record.userId })
        .then((res) => {
          ...
          this.addUserForm.userName = userName || undefined;
          ...
          this.addUserForm.roleIds = roleList.map((item) => item.roleId);
          ...
          this.fileList = profilePath
            ? [{ uid: profile, status: "done", url: profilePath }]
            : [];
      ...
      if (this.addOrChangeUser == "add") {
        ...
        par = {
          ...
          birthday: this.addUserForm.birthday
            ...
            : "",
          userFields: this.formCustomNew.length ? this.TransformData() : undefined, //自定义字段列表
        };
      } else {
        //修改
        par = {
    ...
    toEnable() {
      this.$antConfirm({
        title: "提示",
        content: "重新启用后，仍然是您停用前使用的邀请信息，员工可以继续加入到您的企业。",
        cancelText: "取消",
        onOk: () => {
          StopUsingEnabled({
            codeStatus: 0,
    ...
    showPhoneClick(item, flag) {
      // console.log(item, "itemeeeeeeeeeeee", flag);
      if (!this.canClickBtnMixin("user_phone")) {
        return;
      }
      this.dataList = this.dataList.map((iten) => {
        let obj = { ...iten };
    ...
    async getUserIdentityList() {
      let companyId = (this.companyId = JSON.parse(
        ...
        : "");
      // let productId = JSON.parse(sessionStorage.getItem("zconsole_userInfo")).productSets
      //   ? JSON.parse(sessionStorage.getItem("zconsole_userInfo")).productSets.productId
      //   : "";
    ...
    async getLabelList() {
      ...
      return GetLabelList(postData)
        .then((res) => {
          let { list: labelList, total } = res.data ? res.data : { list: [], total: 0 };
    ...
    TransformData(arr) {
      ...
      let sameArr = objT.map((item) => {
        const data = newArr.find((i) => item.customeFieldsId == i.customeFieldsId);
        return {
          ...item,
          ...data,
        };
    ...
    onExpand(expandedKeys) {
      // console.log('expandedKeysSCSCSCSCS',expandedKeys)
      if (this.userTreeExpand == true) {
        this.userTreeExpand = false;
      }
      // 展开/收起节点时更新expandedKeys
    ...
    getTableList() {
      ...
      console.log("his.selectedKeysOld.length", this.selectedKeysOld.length);
      // 如果展开了，不要deptId ，但是点其他部门的时候还需要这个depeId
      if (this.userTreeExpand == true && this.selectChange) {
        delete postData.deptId;
      }
      return GetUserInfoList(postData) //获取右侧列表接口
        .then((res) => {
          ...
          if (total == 1) {
            ...
            // console.log('主部门',dataList[0].adminDeptAndPosition.split('-')[0]);
            UserDetailInterFace({ userId: dataList[0].userId }).then((res) => {
              // console.log('resss',res.data.deptAndPositionList);
              if (res.data.deptAndPositionList && res.data.deptAndPositionList.length) {
                let deptAndPositionList = res.data.deptAndPositionList;
                deptAndPositionList.forEach((item) => {
                  if (item.adminDept == true) {
                    // console.log('需要展开的节点',item.deptId)
                    // 找到匹配的节点并展开它
                    this.userTreeExpand = true;
                    this.selectChange = true;
                    this.$set(this, "expandedKeys", [item.deptId]);
                    this.selectedKeysOld = [item.deptId];
                  }
                });
              }
            });
