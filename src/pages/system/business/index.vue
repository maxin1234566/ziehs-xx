        ...
        >
          ...
          >
            ...
            <div class="table-btn" slot="action" slot-scope="text, record">
              ...
              <span @click="rmConsoleRoleFn(record)" class="table-btn-rm">删除</span>
            </div>
          </a-table>
        </CommonTable>
      </div>
...
import { getDepartmentTree } from "@/services/company.js";
...
export default {
  ...
  created() {
    ...
    setTimeout(() => {
      ...
      // console.log(this.orgLevelList, "orgLevelListorgLevelListorgLevelList");
    }, 2000);
  },
  methods: {
    modelDeptTree(data) {
    ...
    getRoleItemDetailsFn(apiData) {
      return GetOrgDetail(apiData)
        .then((res) => {
          ...
          console.log(res.data, "编辑回显数据编辑会先数据");
          const parentOrgIds = parentOrgList.map((item) => {
            return {
              label: item.orgName,
              value: item.orgId,
            };
    ...
    changeConsoleRole(item) {
      ...
      this.getDeptList();
      Promise.all([this.getRoleItemDetailsFn(apiData)]).then(() => {
        this.InpDis = true;
    ...
    onSubmit() {
      this.$refs.ruleForm.validate((valid) => {
        if (!valid) return;
        let parentOrgIds = this.formData.parentOrgIds.map((item) => {
          return item.value;
        });
        // let deptIds = this.formData.deptIds.map((item) => {
        //   return item.value;
        // });
        let obj = {
          ...this.formData,
