      ...
      <a-form-model layout="inline" :model="formInline" :colon="false">
        ...
        <a-form-model-item label="状态">
          <a-select v-model="formInline.status" placeholder="请选择状态" allowClear>
            <a-select-option v-for="item in sujectList" :value="item.id" :key="item.id">{{
              item.name
            }}</a-select-option>
          </a-select>
        </a-form-model-item>
      ...
      >
        ...
        <div slot="action" slot-scope="record" class="action">
          ...
          <a-button
            ...
            v-if="record.status != '2'"
            class="color-0067cc cursor-pointer"
            @click="pushStaff(record)"
            >推送</a-button
          >
          <a-button
            type="link"
            class="color-0067cc cursor-pointer"
            @click="actionLook(record)"
            >查看</a-button
          >
          <a-button
            type="link"
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          customRender: (text, record, index) => {
            return record.status == "1" ? "已发布" : "未发布";
          },
        },
        {
          title: "创建人",
  ...
  methods: {
    ...
    actionLook(record) {
      ...
      this.$router.push({
        path: "/ehsGerneralManage/educationmanagement/detailTestManagement",
        query: { dataMsg: record },
      });
    },
    // 预览
    jumpLook(record) {
    ...
    pushConfirm() {
      ...
      console.log(
        ...
        this.$refs.pushModal.chooseStaff
      );
    ...
    publishAction(record) {
      ...
      this.$antConfirm({
        title: "确定发布吗？",
        onOk() {
          TestPublish({ testId: record.testId })
            .then((res) => {
              if (res.code == 20000) {
