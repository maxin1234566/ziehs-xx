<template>
  <div class="clx-show-scroll clx-flex-1 beauty-scroll bg-fff">
        ...
        <SearchTerm>
          <a-form-model layout="inline" :model="formInline" :colon="false">
            ...
            <a-form-model-item class="float-right">
              <a-button type="primary" :loading="loading" @click="iExport">导出</a-button>
    ...
    <CommonDrawer title="考试情况查看" :visible="detailVisible" :cancelFn="detailCancle" :width="'80vw'" :zIndex="2000">
      <template>
        <SearchTerm>
          <a-form-model layout="inline" :model="formInlineDetail" :colon="false">
            ...
            <a-form-model-item class="float-right">
              <a-button type="primary" @click="iExportDetail">导出</a-button>
...
<script>
import { GetSubjectlist, CourseDetail, courseManagementDetail,courseManagementLookDetail, courseManagementDetailExport, PushCodeInfo, courseManagementLookDetailExport, deptDict, courseManagementDetailCourseDowloadApi } from "@/services/api.js";
...
export default {
  ...
  created() {
    ...
    if (!this.dataMsg) {
      this.$router.push("/ehsGerneralManage/educationmanagement/detailTestManagement");
      return;
    }
    GetSubjectlist({
      pageNo: "",
    ...
    CourseDetail({
      ...
      this.dataMsg.subjectId = res.data.subjectId;
    }).catch((err) => {
      console.log(err);
    })
    // deptDict().then(res1 => {
  ...
  methods: {
    ...
    // 导出
    iExport(){
      return courseManagementDetailExport({
        ...this.formInline,
        testId: this.dataMsg.testId,
        pageSize: this.page.pageSize,
        pageNo: this.page.pageNo,
        ...
        downloadElement.download = '推送详情' + '.xlsx';
    ...
    getDataListDetail(record) {
      return courseManagementLookDetail({
        testId: this.dataMsg.testId,
    ...
    //弹框导出
    iExportDetail() {
      return courseManagementLookDetailExport({
        testId: this.dataMsg.testId,
        pushCode: this.detailMsg.pushCode,
        pageSize: this.pageDetail.pageSize,
        pageNo: this.pageDetail.pageNo,
        ...
        const href = URL.createObjectURL(blob); //创建下载链接
        downloadElement.href = href;
        downloadElement.download = '考试情况' + '.xlsx';
        document.body.appendChild(downloadElement);
        downloadElement.click();
        document.body.removeChild(downloadElement);// 下载完成移除元素
        window.URL.revokeObjectURL(href) // 释放掉blob对象
