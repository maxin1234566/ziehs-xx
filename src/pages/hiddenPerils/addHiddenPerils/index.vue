  ...
  <a-spin :spinning="spinning" wrapperClassName="a-spin">
    <HasFixedBottomWrapper>
          ...
          <a-row>
            ...
            <a-col :span="12">
              <a-form-model-item label="发现时间" prop="findTime">
                <a-date-picker
                  v-model="hideDangerForm.findTime"
                  @change="changeFindTime"
                />
              </a-form-model-item>
            </a-col>
            ...
            <a-col :span="12">
              <a-form-model-item label="隐患位置" prop="dangerLocation">
                <a-input
                  v-model.trim="hideDangerForm.dangerLocation"
                  :maxLength="50"
                  placeholder="请输入"
                  allowClear
                />
            ...
            <a-col :span="12">
              <a-form-model-item label="隐患简述" prop="dangerSketch">
                <a-textarea
                  placeholder="请输入"
                  v-model="hideDangerForm.dangerSketch"
                  allowClear
                  :maxLength="50"
                />
              </a-form-model-item>
            ...
            <a-col :span="12">
              <a-form-model-item label="隐患详述" prop="dangerDetail">
                <a-textarea
                  placeholder="请输入"
                  v-model="hideDangerForm.dangerDetail"
                  allowClear
                  :maxLength="500"
                />
              </a-form-model-item>
            ...
            <a-col :span="12">
              <a-form-model-item label="隐患类别" prop="dangerCategory">
                <a-select
                  v-model="hideDangerForm.dangerCategory"
                  placeholder="请选择隐患类别"
                >
                  <a-select-option
            ...
            <a-col :span="12">
              <a-form-model-item label="隐患级别" prop="dangerLevel">
                <a-select
                  v-model="hideDangerForm.dangerLevel"
                  placeholder="请选择隐患类别"
                >
                  <a-select-option
                    v-for="item in troubleClassList"
                    :key="item.key"
                    :value="item.key"
            ...
            <a-col :span="12">
              <StaffOrDept
                ...
                :treePlaceholder="deptTreeId ? '请选择' : '请先选择所属组织部门'"
          ...
          <a-row>
            ...
            <a-col :span="12">
              <a-form-model-item label="隐患整改期限" prop="rectificationTime">
                <a-date-picker
                  v-model="hideDangerForm.rectificationTime"
              ...
              >
                <div>
                  <upload-can-remove
                    ...
                    :headImgs="hideDangerForm.dangerPhotoList"
                  ></upload-can-remove>
                </div>
              </a-form-model-item>
            </a-col>
      ...
      <div slot="fixedBottom">
        <FixedBottom>
          <a-button type="primary" @click="submit('submit')" :loading="loading"
            >提交</a-button
          >
          <a-button class="m-r-10" @click="submit('save')" :loading="loading"
            >保存</a-button
          >
...
import { PushTask, getLiablePeopleByDeptId } from "@/services/api";
...
export default {
  ...
  data() {
    return {
      ...
      addRules: {
        ...
        draftDeptId: [{ required: true, message: "部门不能为空", trigger: "change" }],
        findTime: [
          {
            required: true,
            message: "发现时间不能为空",
        ...
        dangerSketch: [{ required: true, message: "隐患详述不能为空", trigger: "blur" }],
        dangerCategory: [
          { required: true, message: "隐患类别不能为空", trigger: "change" },
        ],
        dangerLevel: [{ required: true, message: "隐患级别不能为空", trigger: "change" }],
  ...
  created() {
    if (sessionStorage.getItem("zconsole_userInfo")) {
      let adminDeptId = JSON.parse(sessionStorage.getItem("zconsole_userInfo")).user
        .adminDeptId;
      let adminDeptName = JSON.parse(sessionStorage.getItem("zconsole_userInfo")).user
        .adminDeptName;
    ...
    this.userObjSession = JSON.parse(sessionStorage.getItem("zconsole_userInfo")).user;
    //起草人
    this.drafterMsg = `${this.userObjSession.name}${
      this.userObjSession.jobNumber ? "/" + this.userObjSession.jobNumber : ""
    }`;
  ...
  methods: {
    ...
    onSearchSelect(value, node, extra) {
      this.deptId = value;
      return getLiablePeopleByDeptId({ deptId: this.deptId }).then((res) => {
    ...
    //   this.deptTreeId = corporationId
    // },

    //责任部门树搜索
    filterTreeNode(inputValue, treeNode) {
      return (
        treeNode.data.props.title && treeNode.data.props.title.indexOf(inputValue) > -1
      );
    },
    ...
    submit(type) {
      ...
      //处理时间
      let findTime = this.hideDangerForm.findTime
        ? dayJs(this.hideDangerForm.findTime).format("YYYY-MM-DD")
        : undefined;
      let rectificationTime = this.hideDangerForm.rectificationTime
        ? dayJs(this.hideDangerForm.rectificationTime).format("YYYY-MM-DD")
        : undefined;
      ...
      let obj = {
        ...
        draftDeptId: this.hideDangerForm.draftDeptId
          ? this.hideDangerForm.draftDeptId
          : this.deptIdList[0],
        draftDeptName: this.hideDangerForm.draftDeptName
          ? this.hideDangerForm.draftDeptName
          : this.deptNameList,
      ...
      //判断是新增还是编辑
      let PromiseThing = this.routerObj.hideDangerId
        ? UpdatehiddenPerilsList
        : AddhiddenPerilsList;
      this.loading = true;
      PromiseThing(obj)
        .then((res) => {
          ...
          this.$router.push(
            "/safeManage/dualControlManage/hiddenPerils/hiddenPerilsList"
          );
        })
        .catch((err) => {
          this.loading = false;
    ...
    deptIdChange(data) {
      let draftDeptId =
        data && data.treeNameAndCodeList.length ? data.treeNameAndCodeList[0].id : "";
    ...
    rectificationResponsibilityPersonIdThing(data) {
      ...
      if (data && data.treeNameAndCodeList.length) {
        GetParentResponsibilityDept({
          ...
          if (res.data) {
            this.hideDangerForm.rectificationResponsibilityDeptName = res.data.deptName;
            this.hideDangerForm.rectificationResponsibilityDeptId = res.data.deptId;
          }
        });
      }
    },
    ...
    getDetail() {
      DetailhiddenPerilsList({ hideDangerId: this.routerObj.hideDangerId })
        .then((res) => {
          ...
          this.deptTreeId = res.data.draftDeptId;
          this.$set(
            this.hideDangerForm,
            "corporationName",
          ...
          if (res.data.dangerPhotoList && res.data.dangerPhotoList.length) {
            // console.log(res.data.dangerPhotoList,'res.data.dangerPhotoListres.data.dangerPhotoList')
            this.hideDangerForm.dangerPhotoList = this.dealImgEcho(
              res.data.dangerPhotoList,
              "fileName",
              "filePath"
            );
          } else {
          ...
          //回显责任人和整改责任人
          this.responsibilityDeptIdList = res.data.responsibilityDeptId
            ? [res.data.responsibilityDeptId]
            : [];
          this.responsibilityPersonIdList = res.data.responsibilityPersonId
            ? [res.data.responsibilityPersonId]
            : [];
          this.rectificationResponsibilityPersonIdList = res.data
            .rectificationResponsibilityPersonId
            ? [res.data.rectificationResponsibilityPersonId]
            : [];
        })
        .catch((err) => {
          console.log(err);
    ...
    disabledDate(current) {
      ...
      if (this.hideDangerForm.findTime) {
        const start = _.cloneDeep(this.hideDangerForm.findTime).subtract(0, "days");
    ...
    addFormUploadSuccess(fileList) {
      ...
      // console.log('文件上传成功fileList',fileList);
