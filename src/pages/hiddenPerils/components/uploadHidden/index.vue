<template>
  <CommonModal title="批量导入" :visible="addVisible" :cancelFn="addCancle">
    ...
    <template slot="btn">
      ...
      <a-button class="m-l-15" type="primary" :loading="loading" @click="okClick"
        >确定</a-button
      >
    </template>
  </CommonModal>
...
export default {
  ...
  data() {
    return {
      ...
      listt: [],
      addFormRules: {
        fileListExel: [{ required: true, message: "文件不能为空", trigger: "change" }],
  ...
  methods: {
    ...
    //下载模板
    downTemplate() {
      DownLoadFiles({ id: '1822116825359335424' }).then((res) => {
        const blob = new Blob([res.data]);
        const objectURL = URL.createObjectURL(blob);
        let btn = document.createElement("a");
        btn.download = "隐患排查导入模板.xlsx";
        btn.href = objectURL;
        btn.click();
