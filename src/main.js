...
import newCommonSearchItem from "@/components/commonSearchItem/newSearch.vue"; // 弹框
...
Vue.component('NewCommonSearchItem', newCommonSearchItem);
...
Vue.filter('userIdFilter', (val, ids) => {
  ...
  if (val && val.length >= 1) {
    ...
    for (let ele of val) {
      const obj = ids[ele];
      text = obj ? text + obj.userName + '、' : text + ''
    }
