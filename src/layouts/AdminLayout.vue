  ...
  <a-layout :class="['admin-layout', 'beauty-scroll', 'height-all']">
    <a-layout class="admin-layout-main beauty-scroll">
      ...
      <a-layout class="admin-layout-main-main">
        ...
        <!-- 主要内容区域 -->
        <a-layout-content
          class="admin-layout-content beauty-scroll-fireBox"
...
export default {
  ...
  data() {
    return {
      ...
      menuShow: true,
      nodeEnv: process.env,
      logoObjCustom: require("@/assets/header/boe.png"),
      // userInfo: {
  ...
  computed: {
    ...
    sideMenuData() {
      ...
      let menuDataS = JSON.parse(sessionStorage.getItem("loginMenu")) || [];
      if (!menuDataS.length) return [];
      // 菜单权限控制
      const result = this.mergeData(this.filterData(menuDataS, data)).map((item) =>
    ...
    userInfo() {
      ...
      obj.company = this.userInfoall.company ? this.userInfoall.company.companyName : ""; //企业名称
  ...
  methods: {
    ...
    dataFlat(params) {
      ...
      params.forEach((item) => {
        item.children
          ? (list = [...list, item, ...this.dataFlat(item.children)])
          : (list = [...list, item]);
      });
      return list;
    },
    dataProcess(node) {
      return {
        ...node.full,
        children: node.full.children.map((item) => this.dataProcess(item)),
      };
    },
    filterData(data1, data2) {
      // 从data2中提取路径
      const names = data1.map((item) => item.resourceName);
      const list = this.dataFlat(data2);
      // 根据名称过滤data1
      const filteredData1 = list.filter((item) => names.includes(item.name));
      return filteredData1.map((item) => {
        let index = 0;
        let localList = list;
        let node = null;
        const nameList = item.fullPath.split("/").filter((item) => item);
        while (index < nameList.length) {
          const curr = localList.find((c) => c.path === nameList[index]);
          if (curr) {
            localList = curr.children || [];
            const current = { ...curr, children: [] };
            node
              ? node.current.children.push({ current: current, full: current })
              : (node = {
                  current: current,
                  full: current,
                });
        ...
        return this.dataProcess(node);
      });
    },
    deleteEmptyChildren(element) {
      if (element.children.length === 0) {
        delete element["children"];
      } else {
        element.children = element.children.map((ele) => this.deleteEmptyChildren(ele));
      }
      return element;
    },
    mergeData(resultList) {
      const record = {};
      resultList.forEach((item) => {
        record[item.path]
          ? record[item.path].children.push(...item.children)
          : (record[item.path] = item);
      });
      return Object.values(record).map((val) =>
        val.children ? { ...val, children: this.mergeData(val.children) } : val
      );
    },
    hoverIn() {
