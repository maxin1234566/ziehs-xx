...
export default {
  props: {
    ...
    accept: {
      type: Array,
      default: () => [".xlsx", ".xls", ".doc", ".docx", ".pdf", ".jpg", ".jpeg", ".png"],
    },
    // 显示文件类型文字
    showAcceptText: {
      type: Boolean,
  ...
  methods: {
    ...
    fileTypeAndSizeTest(file) {
      ...
      // 检测文件类型
      let isLegalType = this.accept.length ? this.accept.includes(fileType) : true;
    ...
    uploadDone(res, file) {
      if (this.multiple) {
        ...
        for (let i = 0; i < this.fileList.length; i++) {
          this.fileList[this.fileList.length - 1].response &&
          this.fileList[this.fileList.length - 1].response.data.id == res.data.id
            ? (ifLast = true)
            : (ifLast = false);
        }
        if (res.data) {
