  ...
  <div style="margin-bottom: 0" :class="{'m-r-15': !notTablePage, 'width-100': notTablePage}">
    <template v-if="!notTablePage">
      <a-form-model-item class="flex" label="所属中心" :colon="false">
        <a-select :disabled="disabled" v-model="CommonFormInline.centerId" placeholder="请选择所属中心" @change="centerIdChange" allowClear>
      ...
      <a-form-model-item class="flex" label="所属组织" :colon="false">
        <a-select :disabled="disabled" v-model="CommonFormInline.corporationId" placeholder="请选择所属组织" @change="corporationChange" allowClear>
    ...
    <template class="width-100" v-else>
      <a-form-model-item v-show="!justNeedDepartment" class="flex" :class="{'ngform-commonItem': isNgForm}" label="所属组织" :colon="isColon" :label-col="labelCol"  :wrapper-col="wrapperCol" prop="corporationId" :label-align="labelAlign">
        <a-select v-model="CommonFormInline.corporationId" placeholder="请选择所属组织" @change="corporationChange" :disabled="disabled" allowClear>
...
export default {
  ...
  created() {
    ...
    // console.log(this.currentRouterCodeMsg,'this.currentRouterCodeMsgthis.currentRouterCodeMsgthis.currentRouterCodeMsg')
    if (currentRouterCodeMsg && (currentRouterCodeMsg.checkedValues != undefined)) {
      if (currentRouterCodeMsg.checkedValues == 1) {// 1法人机构 0： 全部
        ...
        // console.log(this.setCorporationTree,'this.setCorporationTreethis.setCorporationTreethis.setCorporationTreethis.setCorporationTree')
  ...
  methods: {
    ...
    corporationChange(val, rebackDept) { // rebackDept有这个参数的时候，说明是编辑页面需要回显部门
      if (this.notTablePage) { // 新增编辑的时候加组织对应的centerId
        this.$set(this.CommonFormInline, "centerId", this.getMappingValue(this.getCommonAddOrgnizeList, "orgId", val).centerId);
