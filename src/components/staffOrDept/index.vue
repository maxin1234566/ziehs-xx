        ...
        <a-spin :spinning="spinning">
          <a-row>
            ...
            <a-col class="beauty-scroll aCol" :span="14">
              ...
              >
              <!-- :filterTreeNode="filterTreeNode" tree里面的，先注释掉 要不报错 -->
  ...
  methods: {
          ...
          if (res.data) {
            for (let key in res.data) {
              ...
              getAllCheckedList.push({
                ...
                treeName: res.data[key].userName,
                treeCode: res.data[key].jobNumber,
                treeParentId: res.data[key].adminDeptId
              })
              // 回显INPUT框
              reviewShowList.push(res.data[key].userName + (res.data[key].jobNumber ? ('/' + res.data[key].jobNumber) : ''))
            }
    ...
    getDeptAndUser(treeNode) {
      ...
      let apiData = {
        ...
        corporationId: _this.corporationId || '',
        userCount: 0,
        userLimit:999
      }
    ...
    onLoadData(treeNode, getMore) {
      ...
      return new Promise(resolve => {
        if (treeNode != undefined && getMore == undefined && treeNode.dataRef.children && treeNode.dataRef.children.length) {
          this.spinning = false;
          resolve();
          return;
        }
            ...
            if (node.dataRef.children) {
              for (let j = 0; j < node.dataRef.children.length; j++) {
                this.getAllCheckedList.push({
                  ...
                  treeName: node.dataRef.children[j].name,
                  treeParentId: node.dataRef.id,
                  isLeaf: node.dataRef.children[j].isLeaf, // 这里出现了问题
                  treeParentName: node.dataRef.name,
                })
              }
            } else {
    ...
    submitTreeFrom() {
      ...
      // res.data[key].name + (res.data[key].jobNumber ? ('/' + res.data[key].jobNumber):'')
